...
def test_double_click_button(buttons_page: ButtonsPage):
    ...
    with allure.step("Проверяем сообщение о двойном клике"):
        ...
        allure.attach(double_click_message, "double_click_message", allure.attachment_type.TEXT)

        expected_text = "You have done a double click"
        assert expected_text in double_click_message, f"Ожидается текст '{expected_text}', получено '{double_click_message}'"
...
def test_right_click_button(buttons_page: ButtonsPage):
    ...
    with allure.step("Проверяем сообщение о правом клике"):
        ...
        allure.attach(right_click_message, "right_click_message", allure.attachment_type.TEXT)

        expected_text = "You have done a right click"
        assert expected_text in right_click_message, f"Ожидается текст '{expected_text}', получено '{right_click_message}'"
...
def test_regular_click_button(buttons_page: ButtonsPage):
    ...
    with allure.step("Проверяем сообщение об обычном клике"):
        ...
        assert expected_text in click_message, f"Ожидается текст '{expected_text}', получено '{click_message}'"
...
def test_all_button_click_types(buttons_page: ButtonsPage):
    ...
    with allure.step("Анализируем результаты всех кликов"):
        allure.attach(str(click_results), "all_click_results", allure.attachment_type.JSON)
        ...
        assert summary["all_clicks_successful"], f"Все типы кликов должны работать корректно. Успешно: {successful_clicks}/{total_clicks}"
...
def test_button_states_and_visibility(buttons_page: ButtonsPage):
    ...
    with allure.step("Проверяем видимость всех кнопок"):
        ...
        allure.attach(str(buttons_visibility), "buttons_visibility", allure.attachment_type.JSON)
    ...
    with allure.step("Проверяем доступность кнопок для клика"):
        ...
        allure.attach(str(buttons_enabled), "buttons_enabled", allure.attachment_type.JSON)
    ...
    with allure.step("Проверяем текст на кнопках"):
        ...
        for button_name, actual_text in buttons_text.items():
            ...
            assert expected_text.lower() in actual_text.lower(), f"Кнопка '{button_name}' должна содержать текст '{expected_text}'"
