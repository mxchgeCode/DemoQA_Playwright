...
def test_enable_after_button(dynamic_properties_page: DynamicPropertiesPage):
    ...
    with allure.step("Проверяем начальное состояние кнопки 'Enable After 5 Seconds'"):
        ...
        dynamic_properties_page.log_step(f"Кнопка активна изначально: {initial_enabled}")

        assert not initial_enabled, "Кнопка 'Enable After 5 Seconds' должна быть неактивна изначально"
        ...
        allure.attach(str(button_attributes), "initial_button_attributes", allure.attachment_type.JSON)
    ...
    with allure.step("Ожидаем активации кнопки (до 10 секунд)"):
        ...
        button_enabled = dynamic_properties_page.wait_for_enable_after_button(timeout=max_wait_time * 1000)
        ...
        allure.attach(str(timing_info), "button_activation_timing", allure.attachment_type.JSON)

        assert button_enabled, f"Кнопка должна активироваться в течение {max_wait_time} секунд"
    ...
    with allure.step("Проверяем что кнопка стала кликабельной"):
        ...
        dynamic_properties_page.log_step(f"Финальные атрибуты кнопки: {final_attributes}")

        allure.attach(str(final_attributes), "final_button_attributes", allure.attachment_type.JSON)
...
def test_color_change_button(dynamic_properties_page: DynamicPropertiesPage):
    ...
    with allure.step("Получаем начальный цвет кнопки"):
        ...
        allure.attach(str(color_info), "initial_color_info", allure.attachment_type.JSON)

    with allure.step("Ожидаем изменения цвета кнопки"):
        dynamic_properties_page.log_step("Ожидание изменения цвета...")

        max_wait_time = 10  # Максимальное время ожидания
        color_changed = dynamic_properties_page.wait_for_color_change(timeout=max_wait_time * 1000)
        ...
        if color_changed:
            ...
            allure.attach(str(color_change_info), "color_change_result", allure.attachment_type.JSON)

            # Проверяем что действительно произошло изменение
            visual_change = (new_color != initial_color) or (new_classes != initial_classes)
            assert visual_change, f"Должно произойти визуальное изменение кнопки: цвет {initial_color}->{new_color}, классы {initial_classes}->{new_classes}"

        else:
            dynamic_properties_page.log_step("⚠️ Изменение цвета не произошло в ожидаемое время")
    ...
    with allure.step("Проверяем кликабельность кнопки с измененным цветом"):
        ...
        dynamic_properties_page.log_step(f"Кнопка остается кликабельной: {click_result}")

        assert click_result, "Кнопка должна оставаться кликабельной после изменения цвета"
...
def test_visible_after_button(dynamic_properties_page: DynamicPropertiesPage):
    ...
    """
    with allure.step("Проверяем что кнопка 'Visible After 5 Seconds' изначально скрыта"):
        ...
        assert not initial_visible, "Кнопка 'Visible After 5 Seconds' должна быть скрыта изначально"
    ...
    with allure.step("Проверяем наличие кнопки в DOM"):
        ...
        dynamic_properties_page.log_step(f"Кнопка присутствует в DOM: {button_exists_in_dom}")
        ...
        allure.attach(str(visibility_info), "initial_visibility_state", allure.attachment_type.JSON)
    ...
    with allure.step("Ожидаем появления кнопки"):
        ...
        button_appeared = dynamic_properties_page.wait_for_visible_after_button(timeout=max_wait_time * 1000)
        actual_wait_time = time.time() - wait_start_time

        dynamic_properties_page.log_step(f"Время ожидания: {actual_wait_time:.2f} секунд")
        ...
        allure.attach(str(appearance_timing), "button_appearance_timing", allure.attachment_type.JSON)

        assert button_appeared, f"Кнопка должна появиться в течение {max_wait_time} секунд"
    ...
    with allure.step("Проверяем видимость и кликабельность появившейся кнопки"):
        ...
        dynamic_properties_page.log_step(f"Кнопка видима после ожидания: {final_visible}")
        ...
        dynamic_properties_page.log_step(f"Клик по появившейся кнопке успешен: {click_successful}")
        ...
        allure.attach(str(final_state), "final_button_state", allure.attachment_type.JSON)
...
@pytest.mark.elements
@pytest.mark.regression
def test_all_dynamic_properties_simultaneously(dynamic_properties_page: DynamicPropertiesPage):
    """
    ...
    with allure.step(f"Мониторим изменения в течение {test_duration} секунд"):
        ...
        while time.time() - start_time < test_duration:
            ...
            dynamic_properties_page.log_step(f"Состояние на {current_state['timestamp']}с: Enable={current_state['enable_after_enabled']}, Color={current_state['color_change_color']}, Visible={current_state['visible_after_visible']}")

            time.sleep(check_interval)

    with allure.step("Анализируем изменения динамических свойств"):
        allure.attach(str(dynamic_states), "dynamic_states_timeline", allure.attachment_type.JSON)
        ...
        for i in range(1, len(dynamic_states)):
            ...
            if prev_state["color_change_color"] != curr_state["color_change_color"] or prev_state["color_change_classes"] != curr_state["color_change_classes"]:
            ...
            if prev_state["visible_after_visible"] != curr_state["visible_after_visible"]:
        ...
        allure.attach(str(changes_analysis), "changes_analysis", allure.attachment_type.JSON)
    ...
    with allure.step("Проверяем финальное состояние всех элементов"):
        ...
        assert final_checks["enable_after_final"], "Кнопка 'Enable After' должна стать активной"
        assert final_checks["visible_after_final"], "Кнопка 'Visible After' должна стать видимой"
        assert final_checks["total_enable_changes"] > 0, "Должно произойти изменение состояния кнопки 'Enable After'"
        assert final_checks["total_visibility_changes"] > 0, "Должно произойти изменение видимости кнопки 'Visible After'"
...
def test_random_id_element(dynamic_properties_page: DynamicPropertiesPage):
    ...
    with allure.step("Ищем элемент с случайным ID"):
        ...
        dynamic_properties_page.log_step(f"Элемент с случайным ID найден: {random_id_element is not None}")
    ...
    with allure.step("Получаем информацию об элементе с случайным ID"):
        if random_id_element:
            ...
            allure.attach(str(element_info), "random_id_element_info", allure.attachment_type.JSON)

            # Проверяем что ID действительно случайный (содержит цифры/буквы)
            element_id = element_info.get("id", "")
            assert len(element_id) > 5, f"ID должен быть достаточно длинным: {element_id}"

        else:
            dynamic_properties_page.log_step("ℹ️ Элемент с случайным ID не найден на странице")
    ...
    with allure.step("Проверяем стабильность случайного ID"):
        ...
        if random_id_element:
            ...
            allure.attach(str(id_stability), "id_stability_check", allure.attachment_type.JSON)

            # ID должен оставаться стабильным в пределах одной сессии
            assert id_stability["id_stable"], f"ID должен оставаться стабильным в течение сессии: {first_id} != {second_id}"
