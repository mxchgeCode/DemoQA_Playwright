...
def test_switch_to_frame_and_get_text(browser_windows_page: BrowserWindowsPage):
    ...
    with allure.step("Проверяем содержимое фрейма"):
        ...
        assert (
            expected_text in frame_text
        ), f"Ожидается текст '{expected_text}', получено '{frame_text}'"
...
def test_multiple_frames_handling(browser_windows_page: BrowserWindowsPage):
    ...
    with allure.step("Проверяем наличие фреймов на странице"):
        ...
        assert (
            frame_count > 0
        ), f"На странице должен быть хотя бы один фрейм, найдено: {frame_count}"
...
def test_frame_isolation(browser_windows_page: BrowserWindowsPage):
    ...
    with allure.step("Пытаемся найти элементы фрейма из основного контекста"):
        browser_windows_page.log_step(
            "Проверка изоляции: поиск элементов фрейма из основного контекста"
        )

        # Пытаемся найти элементы фрейма не переключаясь в него
        frame_element_accessible = (
            browser_windows_page.is_frame_element_accessible_from_main()
        )
        browser_windows_page.log_step(
            f"Элементы фрейма доступны из основного контекста: {frame_element_accessible}"
        )

        assert (
            not frame_element_accessible
        ), "Элементы фрейма НЕ должны быть доступны из основного контекста"
    ...
    with allure.step("Переключаемся во фрейм и ищем элементы"):
        ...
        browser_windows_page.log_step(
            f"Элементы найдены внутри фрейма: {frame_element_found}"
        )
    ...
    with allure.step("Возвращаемся в основной контент и проверяем изоляцию"):
        ...
        frame_element_accessible_after = (
            browser_windows_page.is_frame_element_accessible_from_main()
        )
        browser_windows_page.log_step(
            f"Элементы фрейма доступны после возврата: {frame_element_accessible_after}"
        )
        ...
        allure.attach(
            str(isolation_result), "frame_isolation_test", allure.attachment_type.JSON
        )
    ...
    with allure.step("Последовательно переключаемся по всем фреймам"):
        for i, frame_info in enumerate(frames_info):
            with allure.step(f"Обрабатываем фрейм {i + 1}"):
                browser_windows_page.log_step(
                    f"Переключение во фрейм {i + 1}: {frame_info}"
                )
                ...
                browser_windows_page.log_step(
                    f"Контент фрейма {i + 1}: {frame_content[:100]}..."
                )
