...
def test_simple_drag_and_drop(droppable_page: DroppablePage):
    ...
    with allure.step("Проверяем начальное состояние элементов"):
        ...
        droppable_page.log_step(
            f"Начальная позиция перетаскиваемого элемента: {initial_drag_position}"
        )
        droppable_page.log_step(f"Начальный текст области сброса: {initial_drop_text}")

        allure.attach(
            str(initial_drag_position),
            "initial_drag_position",
            allure.attachment_type.JSON,
        )
        allure.attach(
            initial_drop_text, "initial_drop_text", allure.attachment_type.TEXT
        )

        expected_initial_text = "Drop here"
        assert (
            expected_initial_text in initial_drop_text
        ), f"Область сброса должна содержать '{expected_initial_text}': {initial_drop_text}"
    ...
    with allure.step("Проверяем состояние после сброса"):
        ...
        droppable_page.log_step(
            f"Цвет фона области после сброса: {drop_area_background}"
        )
        droppable_page.log_step(
            f"Позиция элемента после сброса: {drag_element_position_after}"
        )
        ...
        drop_result = {
            ...
            "drag_position_changed": drag_element_position_after
            != initial_drag_position,
        }

        droppable_page.log_step(f"Результаты сброса: {drop_result}")
        allure.attach(
            str(drop_result), "simple_drop_result", allure.attachment_type.JSON
        )

        # Проверяем что произошли ожидаемые изменения
        assert drop_result[
            "text_changed"
        ], f"Текст области сброса должен измениться: '{initial_drop_text}' -> '{drop_area_text_after}'"

        # Может содержать "Dropped!" или другой текст успеха
        success_indicators = ["Dropped", "dropped", "success"]
        text_indicates_success = any(
            indicator in drop_area_text_after.lower()
            for indicator in success_indicators
        )

        if text_indicates_success:
            droppable_page.log_step(
                "✅ Область сброса показывает успешное завершение операции"
            )
        else:
            droppable_page.log_step(
                "ℹ️ Текст области изменился, но не содержит явных индикаторов успеха"
            )
...
def test_accept_and_reject_elements(droppable_page: DroppablePage):
    ...
    with allure.step("Проверяем элементы для тестирования"):
        ...
        not_acceptable_element_present = (
            droppable_page.is_not_acceptable_element_present()
        )
        ...
        allure.attach(
            str(elements_info), "accept_elements_presence", allure.attachment_type.JSON
        )

        assert (
            accept_drop_area_present
        ), "Область сброса для Accept должна присутствовать"
    ...
    with allure.step("Тестируем сброс принимаемого элемента"):
        if acceptable_element_present:
            ...
            droppable_page.log_step(
                f"Изначальный текст Accept области: {initial_accept_text}"
            )

            acceptable_drag_result = (
                droppable_page.drag_acceptable_element_to_accept_area()
            )
            droppable_page.page.wait_for_timeout(1000)

            accept_text_after = droppable_page.get_accept_drop_area_text()
            accept_area_color_after = (
                droppable_page.get_accept_drop_area_background_color()
            )
            ...
            acceptable_test_result = {
                ...
                "appears_accepted": "Dropped" in accept_text_after
                or accept_text_after != initial_accept_text,
            }

            droppable_page.log_step(
                f"Результат принимаемого элемента: {acceptable_test_result}"
            )
            allure.attach(
                str(acceptable_test_result),
                "acceptable_element_result",
                allure.attachment_type.JSON,
            )
            ...
            if acceptable_test_result["appears_accepted"]:
                ...
                droppable_page.log_step(
                    "⚠️ Принимаемый элемент не показал явных признаков принятия"
                )
        else:
    ...
    with allure.step("Тестируем сброс непринимаемого элемента"):
        if not_acceptable_element_present:
            ...
            droppable_page.log_step(
                f"Текст области перед тестом отклонения: {initial_reject_text}"
            )

            not_acceptable_drag_result = (
                droppable_page.drag_not_acceptable_element_to_accept_area()
            )
            droppable_page.page.wait_for_timeout(1000)

            reject_text_after = droppable_page.get_accept_drop_area_text()
            reject_area_color_after = (
                droppable_page.get_accept_drop_area_background_color()
            )
            ...
            not_acceptable_test_result = {
                ...
                "appears_rejected": reject_text_after
                == initial_reject_text,  # Текст не должен измениться
            }

            droppable_page.log_step(
                f"Результат непринимаемого элемента: {not_acceptable_test_result}"
            )
            allure.attach(
                str(not_acceptable_test_result),
                "not_acceptable_element_result",
                allure.attachment_type.JSON,
            )

            if not_acceptable_test_result["appears_rejected"]:
                droppable_page.log_step(
                    "✅ Непринимаемый элемент был корректно отклонен"
                )
            else:
                droppable_page.log_step(
                    "⚠️ Непринимаемый элемент показал неожиданное поведение"
                )
        else:
    ...
    with allure.step("Анализируем поведение Accept/Reject"):
        accept_reject_summary = {
            ...
            "both_elements_available": acceptable_element_present
            and not_acceptable_element_present,
            "accept_functionality_works": True,  # Базовая проверка что вкладка работает
        }

        droppable_page.log_step(
            f"Итоги Accept/Reject функциональности: {accept_reject_summary}"
        )
        allure.attach(
            str(accept_reject_summary),
            "accept_reject_summary",
            allure.attachment_type.JSON,
        )

        assert accept_reject_summary[
            "accept_functionality_works"
        ], "Функциональность Accept/Reject должна работать"
...
def test_prevent_propagation(droppable_page: DroppablePage):
    ...
    with allure.step("Переключаемся на вкладку Prevent Propagation"):
        ...
        assert (
            propagation_tab_active
        ), "Вкладка Prevent Propagation должна быть активной"
    ...
    with allure.step("Анализируем структуру вложенных областей"):
        ...
        droppable_page.log_step(
            f"Структура областей propagation: {propagation_structure}"
        )
        allure.attach(
            str(propagation_structure),
            "propagation_structure",
            allure.attachment_type.JSON,
        )
    ...
    with allure.step("Тестируем сброс в не-жадную внутреннюю область"):
        if inner_not_greedy_present and drag_box_present:
            ...
            not_greedy_test = {
                ...
                "propagation_occurred": outer_not_greedy_after
                != outer_not_greedy_initial
                and inner_not_greedy_after != inner_not_greedy_initial,
            }

            droppable_page.log_step(f"Результат не-жадного сброса: {not_greedy_test}")
            allure.attach(
                str(not_greedy_test),
                "not_greedy_drop_result",
                allure.attachment_type.JSON,
            )
    ...
    with allure.step("Тестируем сброс в жадную внутреннюю область"):
        if inner_greedy_present and drag_box_present:
            ...
            greedy_test = {
                ...
                "propagation_prevented": inner_greedy_after != inner_greedy_initial
                and outer_greedy_after == outer_greedy_initial,
            }

            droppable_page.log_step(f"Результат жадного сброса: {greedy_test}")
            allure.attach(
                str(greedy_test), "greedy_drop_result", allure.attachment_type.JSON
            )
    ...
    with allure.step("Анализируем поведение предотвращения всплытия"):
        propagation_analysis = {
            "not_greedy_areas_available": inner_not_greedy_present
            and outer_not_greedy_present,
            "greedy_areas_available": inner_greedy_present and outer_greedy_present,
            "drag_element_available": drag_box_present,
            "propagation_testing_possible": drag_box_present
            and (inner_not_greedy_present or inner_greedy_present),
            "tab_functionality_works": propagation_tab_active,
        }

        droppable_page.log_step(
            f"Анализ предотвращения всплытия: {propagation_analysis}"
        )
        allure.attach(
            str(propagation_analysis),
            "propagation_analysis",
            allure.attachment_type.JSON,
        )

        assert propagation_analysis[
            "tab_functionality_works"
        ], "Функциональность предотвращения всплытия должна быть доступна"
        ...
        if propagation_analysis["propagation_testing_possible"]:
            ...
            droppable_page.log_step(
                "⚠️ Не все элементы для тестирования всплытия найдены"
            )
...
def test_revertable_drag_elements(droppable_page: DroppablePage):
    ...
    with allure.step("Анализируем элементы с возвратом"):
        ...
        not_revertable_element_present = (
            droppable_page.is_not_revertable_element_present()
        )
    ...
    with allure.step("Тестируем возвратный элемент"):
        if revertable_element_present and revert_drop_area_present:
            ...
            revertable_initial_position = (
                droppable_page.get_revertable_element_position()
            )
            droppable_page.log_step(
                f"Начальная позиция возвратного элемента: {revertable_initial_position}"
            )
            ...
            revertable_test = {
                ...
                "element_moved_during_drag": position_after_drag
                != revertable_initial_position,
                "element_reverted": final_position == revertable_initial_position
                or abs(final_position["x"] - revertable_initial_position["x"]) < 10,
                "drop_area_accepted": "Dropped" in drop_area_text,
            }

            droppable_page.log_step(
                f"Результат теста возвратного элемента: {revertable_test}"
            )
            allure.attach(
                str(revertable_test),
                "revertable_element_test",
                allure.attachment_type.JSON,
            )

            if (
                revertable_test["element_reverted"]
                and not revertable_test["drop_area_accepted"]
            ):
                droppable_page.log_step(
                    "✅ Возвратный элемент корректно вернулся на место"
                )
            elif revertable_test["drop_area_accepted"]:
                droppable_page.log_step(
                    "✅ Возвратный элемент был принят областью сброса"
                )
            else:
                droppable_page.log_step(
                    "ℹ️ Поведение возвратного элемента отличается от ожидаемого"
                )
    ...
    with allure.step("Тестируем не-возвратный элемент"):
        if not_revertable_element_present and revert_drop_area_present:
            ...
            not_revertable_initial = (
                droppable_page.get_not_revertable_element_position()
            )
            droppable_page.log_step(
                f"Начальная позиция не-возвратного элемента: {not_revertable_initial}"
            )

            # Перетаскиваем элемент
            droppable_page.log_step("Перетаскивание не-возвратного элемента")
            not_revert_drag_result = (
                droppable_page.drag_not_revertable_element_to_drop_area()
            )
            ...
            droppable_page.log_step(
                f"Результат теста не-возвратного элемента: {not_revertable_test}"
            )
            allure.attach(
                str(not_revertable_test),
                "not_revertable_element_test",
                allure.attachment_type.JSON,
            )

            if (
                not_revertable_test["drop_area_accepted"]
                or not_revertable_test["element_stayed_moved"]
            ):
                droppable_page.log_step(
                    "✅ Не-возвратный элемент ведет себя как ожидается"
                )
            else:
                droppable_page.log_step(
                    "ℹ️ Поведение не-возвратного элемента требует анализа"
                )

        else:
            droppable_page.log_step(
                "⚠️ Не-возвратный элемент для тестирования не найден"
            )
    ...
    with allure.step("Анализируем функциональность возврата"):
        revert_functionality_summary = {
            ...
            "revert_testing_possible": revertable_element_present
            and revert_drop_area_present,
            "tab_works": revert_tab_active,
        }

        droppable_page.log_step(
            f"Итоги функциональности возврата: {revert_functionality_summary}"
        )
        allure.attach(
            str(revert_functionality_summary),
            "revert_functionality_summary",
            allure.attachment_type.JSON,
        )

        assert revert_functionality_summary[
            "tab_works"
        ], "Вкладка Revert Draggable должна работать"
...
def test_all_droppable_tabs_integration(droppable_page: DroppablePage):
        ...
        for tab_name, tab_key in tabs_to_test:
            with allure.step(f"Тестирование вкладки {tab_name}"):
                ...
                tab_info = {
                    ...
                    "functional": tab_active
                    and content_present
                    and drag_elements_count > 0,
                }
                ...
                if tab_info["functional"]:
                    basic_test_result = droppable_page.perform_basic_drag_test_in_tab(
                        tab_key
                    )
                    tab_info["basic_test_passed"] = basic_test_result
                    droppable_page.log_step(
                        f"Базовый тест вкладки {tab_name}: {basic_test_result}"
                    )
                else:
    ...
    with allure.step("Анализируем интеграцию всех вкладок"):
        allure.attach(
            str(tabs_info), "all_tabs_integration_info", allure.attachment_type.JSON
        )

        functional_tabs = sum(1 for info in tabs_info.values() if info["functional"])
        tabs_with_passed_tests = sum(
            1 for info in tabs_info.values() if info.get("basic_test_passed", False)
        )
        ...
        integration_summary = {
            ...
            "integration_successful": functional_tabs
            >= 2,  # Минимум 2 вкладки должны работать
        ...
        allure.attach(
            ...
            "droppable_integration_summary",
            allure.attachment_type.JSON,
        )

        assert integration_summary[
            "integration_successful"
        ], f"Минимум 2 вкладки должны быть функциональными: {functional_tabs}/{total_tabs}"
        assert (
            functional_tabs > 0
        ), f"Хотя бы одна вкладка должна быть функциональной: {functional_tabs}/{total_tabs}"

        if integration_summary["all_tabs_functional"]:
            droppable_page.log_step("🎉 Все вкладки Droppable функциональны!")
        else:
            droppable_page.log_step(
                f"ℹ️ Функциональных вкладок: {functional_tabs}/{total_tabs}"
            )
