...
def test_resize_element_with_restrictions(resizable_page: ResizablePage):
    ...
    with allure.step("Получаем начальные размеры элемента с ограничениями"):
        ...
        allure.attach(
            str(initial_size), "initial_restricted_size", allure.attachment_type.JSON
        )

        assert (
            initial_size["width"] > 0
        ), f"Начальная ширина должна быть больше 0: {initial_size['width']}"
        assert (
            initial_size["height"] > 0
        ), f"Начальная высота должна быть больше 0: {initial_size['height']}"
    ...
    with allure.step("Получаем ограничения размеров"):
        ...
        allure.attach(
            str(size_restrictions), "size_restrictions", allure.attachment_type.JSON
        )
    ...
    with allure.step("Проверяем новые размеры после увеличения"):
        ...
        size_change_increase = {
            ...
            "height_changed": new_size_after_increase["height"]
            != initial_size["height"],
            "width_increase": new_size_after_increase["width"] - initial_size["width"],
            "height_increase": new_size_after_increase["height"]
            - initial_size["height"],
        }

        resizable_page.log_step(f"Изменения при увеличении: {size_change_increase}")
        allure.attach(
            str(size_change_increase),
            "size_increase_result",
            allure.attachment_type.JSON,
        )

        # Проверяем что размер изменился
        size_actually_changed = (
            size_change_increase["width_changed"]
            or size_change_increase["height_changed"]
        )

        assert (
            size_actually_changed
        ), f"Размер должен измениться при увеличении: {size_change_increase}"
    ...
    with allure.step("Пытаемся уменьшить размер элемента"):
        ...
        size_change_decrease = {
            ...
            "width_decreased": new_size_after_decrease["width"]
            < new_size_after_increase["width"],
            "height_decreased": new_size_after_decrease["height"]
            < new_size_after_increase["height"],
            "within_min_limits": (
                new_size_after_decrease["width"]
                >= size_restrictions.get("min_width", 0)
                and new_size_after_decrease["height"]
                >= size_restrictions.get("min_height", 0)
            ),
        }

        resizable_page.log_step(f"Изменения при уменьшении: {size_change_decrease}")
        allure.attach(
            str(size_change_decrease),
            "size_decrease_result",
            allure.attachment_type.JSON,
        )

        # Проверяем что ограничения соблюдаются
        if size_restrictions.get("min_width") and size_restrictions.get("min_height"):
            assert size_change_decrease[
                "within_min_limits"
            ], "Размеры не должны нарушать минимальные ограничения"
...
def test_resize_element_without_restrictions(resizable_page: ResizablePage):
    ...
    with allure.step("Получаем начальные размеры свободного элемента"):
        ...
        resizable_page.log_step(
            f"Начальный размер свободного элемента: {initial_free_size}"
        )

        allure.attach(
            str(initial_free_size), "initial_free_size", allure.attachment_type.JSON
        )

        assert (
            initial_free_size["width"] > 0
        ), "Начальная ширина свободного элемента должна быть больше 0"
        assert (
            initial_free_size["height"] > 0
        ), "Начальная высота свободного элемента должна быть больше 0"
    ...
    with allure.step("Значительно увеличиваем размер свободного элемента"):
        ...
        resizable_page.log_step(
            f"Размер после значительного увеличения: {size_after_large_increase}"
        )

        large_increase_change = {
            "initial_size": initial_free_size,
            "size_after_increase": size_after_large_increase,
            "width_change": size_after_large_increase["width"]
            - initial_free_size["width"],
            "height_change": size_after_large_increase["height"]
            - initial_free_size["height"],
            "significant_increase": (
                size_after_large_increase["width"] > initial_free_size["width"] + 50
                or size_after_large_increase["height"]
                > initial_free_size["height"] + 50
            ),
        }

        resizable_page.log_step(
            f"Результат значительного увеличения: {large_increase_change}"
        )
        allure.attach(
            str(large_increase_change),
            "large_increase_result",
            allure.attachment_type.JSON,
        )

        assert large_increase_change[
            "significant_increase"
        ], "Свободный элемент должен значительно увеличиваться"
    ...
    with allure.step("Значительно уменьшаем размер свободного элемента"):
        ...
        resizable_page.log_step(
            f"Размер после значительного уменьшения: {size_after_large_decrease}"
        )

        large_decrease_change = {
            "size_before_decrease": size_after_large_increase,
            "size_after_decrease": size_after_large_decrease,
            "width_decrease": size_after_large_increase["width"]
            - size_after_large_decrease["width"],
            "height_decrease": size_after_large_increase["height"]
            - size_after_large_decrease["height"],
            "significant_decrease": (
                size_after_large_decrease["width"]
                < size_after_large_increase["width"] - 50
                or size_after_large_decrease["height"]
                < size_after_large_increase["height"] - 50
            ),
        }

        resizable_page.log_step(
            f"Результат значительного уменьшения: {large_decrease_change}"
        )
        allure.attach(
            str(large_decrease_change),
            "large_decrease_result",
            allure.attachment_type.JSON,
        )

        assert large_decrease_change[
            "significant_decrease"
        ], "Свободный элемент должен значительно уменьшаться"
    ...
    with allure.step("Проверяем финальные размеры"):
        ...
        resize_summary = {
            ...
            "resize_operations_successful": large_resize_result
            and large_decrease_result,
        }

        resizable_page.log_step(f"Итоги изменения размеров: {resize_summary}")
        allure.attach(
            str(resize_summary), "free_resize_summary", allure.attachment_type.JSON
        )

        assert resize_summary[
            "resize_operations_successful"
        ], "Все операции изменения размера должны быть успешными"
...
def test_resize_handles_functionality(resizable_page: ResizablePage):
    ...
    with allure.step("Тестируем различные ручки изменения размера"):
        ...
        initial_size = resizable_page.get_restricted_box_size()
        resizable_page.log_step(
            f"Начальный размер для тестирования ручек: {initial_size}"
        )

        for handle_position, handle_description in handles_to_test:
            with allure.step(f"Тестируем {handle_description}"):
                resizable_page.log_step(
                    f"Тест ручки: {handle_description} ({handle_position})"
                )

                # Проверяем видимость ручки
                handle_visible = resizable_page.is_resize_handle_visible(
                    handle_position
                )

                if handle_visible:
                    # Пытаемся изменить размер через эту ручку
                    handle_resize_result = resizable_page.resize_by_handle(
                        handle_position, 50, 50
                    )
                ...
                resizable_page.log_step(
                    f"Результат тестирования ручки {handle_position}: {handle_test_result}"
                )

    with allure.step("Анализируем функциональность ручек"):
        allure.attach(
            str(handle_tests), "handle_tests_results", allure.attachment_type.JSON
        )
        ...
        handles_that_changed_size = sum(
            1 for test in handle_tests if test["size_changed"]
        )
        ...
        allure.attach(
            str(handles_summary),
            "handles_functionality_summary",
            allure.attachment_type.JSON,
        )

        # Проверяем что хотя бы одна ручка работает
        assert handles_summary[
            "any_handle_works"
        ], f"Хотя бы одна ручка должна работать: работающих {working_handles}/{len(handle_tests)}"

        # Проверяем что есть видимые ручки
        assert (
            visible_handles > 0
        ), f"Должна быть видима хотя бы одна ручка: видимых {visible_handles}/{len(handle_tests)}"
...
def test_resize_size_validation(resizable_page: ResizablePage):
    ...
    with allure.step("Получаем информацию о размерах элементов"):
        ...
        resizable_page.log_step(
            f"Информация об ограниченном элементе: {restricted_info}"
        )
        resizable_page.log_step(f"Информация о свободном элементе: {free_info}")

        allure.attach(
            str(restricted_info), "restricted_element_info", allure.attachment_type.JSON
        )
    ...
    with allure.step("Тестируем валидность текущих размеров"):
        ...
        restricted_size_valid = (
            restricted_info["current_size"]["width"] > 0
            and restricted_info["current_size"]["height"] > 0
            and restricted_info["current_size"]["width"] < 2000  # Разумные пределы
            and restricted_info["current_size"]["height"] < 2000
        )

        free_size_valid = (
            free_info["current_size"]["width"] > 0
            and free_info["current_size"]["height"] > 0
            and free_info["current_size"]["width"] < 2000
            and free_info["current_size"]["height"] < 2000
        )
        ...
        resizable_page.log_step(
            f"Валидация текущих размеров: {current_sizes_validation}"
        )
    ...
    with allure.step("Тестируем соблюдение ограничений"):
        ...
        if limits.get("min_width"):
            limits_compliance["width_within_limits"] = (
                current_restricted_size["width"] >= limits["min_width"]
            )
        if limits.get("max_width"):
            limits_compliance["width_within_limits"] = (
                limits_compliance["width_within_limits"]
                and current_restricted_size["width"] <= limits["max_width"]
            )

        # Проверяем соблюдение ограничений по высоте
        if limits.get("min_height"):
            limits_compliance["height_within_limits"] = (
                current_restricted_size["height"] >= limits["min_height"]
            )
        if limits.get("max_height"):
            limits_compliance["height_within_limits"] = (
                limits_compliance["height_within_limits"]
                and current_restricted_size["height"] <= limits["max_height"]
            )

        limits_compliance["all_limits_respected"] = (
            limits_compliance["width_within_limits"]
            and limits_compliance["height_within_limits"]
        )
    ...
    with allure.step("Тестируем экстремальные изменения размеров"):
        ...
        extreme_resize_test = {
            ...
            "prevented_too_small": (
                size_after_extreme_small["width"] > 10
                and size_after_extreme_small["height"] > 10
            ),
            "handled_extreme_large": (
                size_after_extreme_large["width"] < 2000
                and size_after_extreme_large["height"] < 2000
            ),
        }

        validation_tests.append(("extreme_resize", extreme_resize_test))
        resizable_page.log_step(
            f"Экстремальные изменения размеров: {extreme_resize_test}"
        )

    with allure.step("Создаем итоговый отчет валидации"):
        allure.attach(
            str(validation_tests), "size_validation_tests", allure.attachment_type.JSON
        )
        ...
        validation_summary = {
            ...
            "limits_respected": (
                validation_tests[1][1]["all_limits_respected"]
                if len(validation_tests) > 1
                else True
            ),
            "extreme_cases_handled": (
                validation_tests[2][1]["prevented_too_small"]
        ...
        validation_summary["overall_validation_passed"] = (
            validation_summary["current_sizes_valid"]
            and validation_summary["limits_respected"]
            and validation_summary["extreme_cases_handled"]
        )

        resizable_page.log_step(f"Итоги валидации размеров: {validation_summary}")
        allure.attach(
            str(validation_summary),
            "size_validation_summary",
            allure.attachment_type.JSON,
        )

        assert validation_summary[
            "current_sizes_valid"
        ], "Текущие размеры должны быть валидными"
        assert validation_summary[
            "overall_validation_passed"
        ], "Общая валидация размеров должна пройти успешно"
...
def test_resizable_responsive_behavior(resizable_page: ResizablePage):
    ...
    with allure.step("Тестируем поведение при различных размерах окна"):
        for width, height, description in viewport_sizes:
            with allure.step(f"Тест для {description} ({width}x{height})"):
                resizable_page.log_step(
                    f"Изменение размера окна на {description}: {width}x{height}"
                )

                # Изменяем размер окна браузера
                resizable_page.page.set_viewport_size(
                ...
                viewport_test = {
                    ...
                    "elements_fit_viewport": (
                        restricted_size_viewport["width"] <= width
                        and restricted_size_viewport["height"] <= height
                        and free_size_viewport["width"] <= width
                        and free_size_viewport["height"] <= height
                    ),
                }
    ...
    with allure.step("Анализируем отзывчивое поведение"):
        allure.attach(
            str(viewport_tests), "viewport_tests_results", allure.attachment_type.JSON
        )

        viewports_with_visible_elements = sum(
            1
            for test in viewport_tests
            if test["restricted_visible"] and test["free_visible"]
        )
        viewports_with_working_resize = sum(
            1 for test in viewport_tests if test["resize_works"]
        )
        viewports_with_fitting_elements = sum(
            1 for test in viewport_tests if test["elements_fit_viewport"]
        )
        ...
        responsive_summary = {
            ...
            "responsive_behavior_good": viewports_with_visible_elements
            >= len(viewport_tests) // 2,
            "resize_works_across_viewports": viewports_with_working_resize
            >= len(viewport_tests) // 2,
        ...
        allure.attach(
            str(responsive_summary),
            "responsive_behavior_summary",
            allure.attachment_type.JSON,
        )

        # Проверяем что элементы ведут себя разумно в большинстве viewport
        assert responsive_summary[
            "responsive_behavior_good"
        ], f"Элементы должны быть видимы в большинстве размеров экрана: {viewports_with_visible_elements}/{len(viewport_tests)}"
        assert responsive_summary[
            "resize_works_across_viewports"
        ], f"Изменение размеров должно работать в большинстве viewport: {viewports_with_working_resize}/{len(viewport_tests)}"
