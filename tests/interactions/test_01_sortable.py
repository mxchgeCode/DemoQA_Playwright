...
def test_sort_list_items(sortable_page: SortablePage):
    ...
    with allure.step("Получаем изначальный порядок элементов списка"):
        ...
        allure.attach(str(initial_order), "initial_list_order", allure.attachment_type.JSON)

        assert len(initial_order) >= 3, f"В списке должно быть минимум 3 элемента, найдено: {len(initial_order)}"
    ...
    with allure.step("Проверяем новый порядок после перетаскивания"):
        ...
        assert order_changed, f"Порядок элементов должен измениться: было {initial_order}, стало {new_order}"

        # Проверяем что первый элемент переместился
        if len(initial_order) >= 3 and len(new_order) >= 3:
            first_element_moved = initial_order[0] not in new_order[:1]  # Первый элемент больше не на первой позиции
            sortable_page.log_step(f"Первый элемент переместился: {first_element_moved}")
        ...
        allure.attach(str(sorting_result), "list_sorting_result", allure.attachment_type.JSON)
...
def test_sort_grid_items(sortable_page: SortablePage):
    ...
    with allure.step("Получаем изначальный порядок элементов сетки"):
        ...
        allure.attach(str(initial_grid_order), "initial_grid_order", allure.attachment_type.JSON)

        assert len(initial_grid_order) >= 6, f"В сетке должно быть минимум 6 элементов, найдено: {len(initial_grid_order)}"
    ...
    with allure.step("Проверяем изменения в сетке"):
        ...
        allure.attach(str(new_grid_order), "new_grid_order", allure.attachment_type.JSON)

        grid_order_changed = new_grid_order != initial_grid_order
        sortable_page.log_step(f"Порядок сетки изменился: {grid_order_changed}")

        assert grid_order_changed, f"Порядок элементов сетки должен измениться: было {initial_grid_order}, стало {new_grid_order}"
        ...
        allure.attach(str(grid_sorting_result), "grid_sorting_result", allure.attachment_type.JSON)
...
def test_multiple_sort_operations(sortable_page: SortablePage):
    ...
    with allure.step("Выполняем серию операций сортировки в списке"):
        ...
        for move_index, (from_pos, to_pos) in enumerate(moves_to_perform, 1):
            with allure.step(f"Перетаскивание {move_index}: позиция {from_pos} → {to_pos}"):
                sortable_page.log_step(f"Операция {move_index}: перемещение с позиции {from_pos} на {to_pos}")

                # Получаем порядок до операции
                order_before = sortable_page.get_list_items_order()

                # Выполняем перетаскивание
                drag_success = sortable_page.drag_list_item_to_position(from_pos, to_pos)
                ...
                sortable_page.log_step(f"Результат операции {move_index}: {move_result}")
    ...
    with allure.step("Анализируем результаты множественных операций"):
        ...
        sortable_page.log_step(f"Итоги множественных операций: {multiple_moves_summary}")
        allure.attach(str(multiple_moves_summary), "multiple_moves_summary", allure.attachment_type.JSON)

        # Проверяем что хотя бы некоторые операции были успешными
        assert successful_moves > 0, f"Хотя бы одна операция перетаскивания должна быть успешной: {successful_moves}/{len(moves_log)}"

        # Проверяем что финальный порядок отличается от изначального
        assert multiple_moves_summary["total_order_change"], f"После серии операций порядок должен измениться: {initial_order} -> {final_order}"
...
def test_tab_switching_preserves_state(sortable_page: SortablePage):
    ...
    with allure.step("Возвращаемся на вкладку List и проверяем сохранность"):
        ...
        list_state_preserved = (
            current_list_order == tab_states["list"]["modified_order"] or
            current_list_order != tab_states["list"]["initial_order"]
        )
    ...
    with allure.step("Анализируем поведение переключения вкладок"):
        allure.attach(str(tab_states), "tab_states_analysis", allure.attachment_type.JSON)
        ...
        allure.attach(str(tab_switching_summary), "tab_switching_summary", allure.attachment_type.JSON)

        # Проверяем что переключения между вкладками работают
        assert tab_switching_summary["tab_switching_works"], "Переключение между вкладками должно работать"
...
def test_sortable_elements_validation(sortable_page: SortablePage):
    ...
    with allure.step("Валидация элементов списка"):
        ...
        list_validation = {
            ...
            "elements_visible": all(elem.get("visible", False) for elem in list_elements),
            "elements_have_classes": all(elem.get("classes") for elem in list_elements),
            "unique_texts": len(set(elem["text"] for elem in list_elements)) == len(list_elements)
        }
        ...
        assert list_validation["total_elements"] >= 3, f"В списке должно быть минимум 3 элемента: {list_validation['total_elements']}"
        assert list_validation["elements_have_text"], "Все элементы списка должны иметь текст"
        assert list_validation["elements_visible"], "Все элементы списка должны быть видимыми"
    ...
    with allure.step("Валидация элементов сетки"):
        ...
        grid_validation = {
            ...
            "elements_visible": all(elem.get("visible", False) for elem in grid_elements),
            "elements_have_classes": all(elem.get("classes") for elem in grid_elements),
            "unique_texts": len(set(elem["text"] for elem in grid_elements)) == len(grid_elements),
        ...
        assert grid_validation["total_elements"] >= 6, f"В сетке должно быть минимум 6 элементов: {grid_validation['total_elements']}"
        assert grid_validation["elements_have_text"], "Все элементы сетки должны иметь текст"
        assert grid_validation["elements_visible"], "Все элементы сетки должны быть видимыми"
    ...
    with allure.step("Проверяем интерактивность элементов"):
        ...
        allure.attach(str(validation_results), "elements_validation_results", allure.attachment_type.JSON)
        ...
        assert overall_validation["interactive_elements"], "Элементы должны быть интерактивными (draggable или hoverable)"
