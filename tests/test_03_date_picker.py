...
def test_date_time_picker_opens(datepicker_page):
    ...
    assert datepicker_page.time_picker_list.is_visible(), "Список времени должен быть видим"
...
def test_date_picker_input_fields_functionality(datepicker_page):
    ...
    assert date_value_after != date_value_before, "Дата должна измениться после выбора другой даты"
...
def test_date_picker_navigation(datepicker_page):
    ...
    assert prev_month == initial_month, "Месяц должен вернуться к исходному после перехода назад"
...
def test_date_picker_month_year_selection(datepicker_page):
    ...
    assert datepicker_page.calendar_month_select.is_visible(), "Dropdown месяца должен быть доступен"
    assert datepicker_page.calendar_year_select.is_visible(), "Dropdown года должен быть доступен"
