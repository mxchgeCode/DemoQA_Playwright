...
def test_basic_tab_switching(tabs_page: TabsPage):
    ...
    with allure.step("Получаем список всех вкладок"):
        ...
        assert (
            len(all_tabs) >= 2
        ), f"Должно быть минимум 2 вкладки для тестирования, найдено: {len(all_tabs)}"
    ...
    with allure.step("Переключаемся на каждую вкладку"):
        for i, tab_info in enumerate(all_tabs):
            ...
            with allure.step(
                f"Переключение на вкладку: '{tab_title}' (индекс {tab_index})"
            ):
                ...
                switching_test = {
                    ...
                    "became_active": active_after
                    and active_after["index"] == tab_index,
                    "content_visible": tab_content_visible,
                    "switching_successful": click_result
                    and active_after
                    and active_after["index"] == tab_index,
                }
                ...
                tabs_page.log_step(
                    f"Результат переключения на '{tab_title}': {switching_test}"
                )
    ...
    with allure.step("Анализируем результаты переключения вкладок"):
        allure.attach(
            str(tab_switching_results),
            "tab_switching_results",
            allure.attachment_type.JSON,
        )

        successful_switches = sum(
            1 for result in tab_switching_results if result["switching_successful"]
        )
        tabs_with_visible_content = sum(
            1 for result in tab_switching_results if result["content_visible"]
        )
        ...
        switching_summary = {
            ...
            "switching_success_rate": (
                successful_switches / len(tab_switching_results)
                if tab_switching_results
                else 0
            ),
            "content_visibility_rate": (
                tabs_with_visible_content / len(tab_switching_results)
                if tab_switching_results
                else 0
            ),
            "tab_switching_works": successful_switches
            >= len(tab_switching_results) * 0.8,
            "content_display_works": tabs_with_visible_content
            >= len(tab_switching_results) * 0.8,
        }

        tabs_page.log_step(f"Итоги переключения вкладок: {switching_summary}")
        allure.attach(
            str(switching_summary), "tab_switching_summary", allure.attachment_type.JSON
        )

        assert switching_summary[
            "tab_switching_works"
        ], f"Переключение вкладок должно работать: {successful_switches}/{len(tab_switching_results)}"
...
def test_tab_content_display(tabs_page: TabsPage):
    ...
    with allure.step("Анализируем содержимое каждой вкладки"):
        ...
        for tab_info in all_tabs:
            ...
            with allure.step(f"Анализ содержимого вкладки: '{tab_title}'"):
                tabs_page.log_step(
                    f"Переключение на '{tab_title}' для анализа содержимого"
                )
                ...
                content_elements_count = tabs_page.count_elements_in_tab_content(
                    tab_index
                )
                ...
                content_analysis = {
                    ...
                    "tab_index": tab_index,
                    "content_text": (
                        tab_content_text[:200] + "..."
                        if len(tab_content_text) > 200
                        else tab_content_text
                    ),
                    ...
                    "content_preview": (
                        tab_content_text[:100] if tab_content_text else ""
                    ),
                }
    ...
    with allure.step("Проверяем уникальность содержимого вкладок"):
        allure.attach(
            str(tabs_content_info), "tabs_content_analysis", allure.attachment_type.JSON
        )
        ...
        meaningful_content_count = sum(
            1 for info in tabs_content_info if info["has_meaningful_content"]
        )
        visible_content_count = sum(
            1 for info in tabs_content_info if info["content_visible"]
        )

        # Анализ разнообразия элементов
        different_element_counts = len(
            set(info["elements_count"] for info in tabs_content_info)
        )
        ...
        content_uniqueness_analysis = {
            ...
            "most_tabs_have_content": meaningful_content_count
            >= len(tabs_content_info) * 0.8,
            "content_visibility_good": visible_content_count
            >= len(tabs_content_info) * 0.8,
        ...
        assert content_uniqueness_analysis[
            "most_tabs_have_content"
        ], f"Большинство вкладок должны содержать значимый контент: {meaningful_content_count}/{len(tabs_content_info)}"
        assert content_uniqueness_analysis[
            "content_visibility_good"
        ], f"Содержимое должно быть видимым: {visible_content_count}/{len(tabs_content_info)}"

        if content_uniqueness_analysis["all_contents_unique"]:
            tabs_page.log_step("✅ Все вкладки содержат уникальный контент")
        else:
            tabs_page.log_step(
                "ℹ️ Некоторые вкладки могут содержать повторяющийся контент"
            )
...
def test_tab_active_inactive_states(tabs_page: TabsPage):
        ...
        for tab_info in all_tabs:
            ...
            with allure.step(f"Анализ состояний вкладки: '{tab_title}'"):
                ...
                state_analysis = {
                    ...
                    "states_distinguish": inactive_state != active_state
                    or inactive_classes != active_classes,
                }
    ...
    with allure.step("Проверяем различимость активного/неактивного состояний"):
        allure.attach(
            str(tab_states_analysis), "tab_states_analysis", allure.attachment_type.JSON
        )

        distinguishable_states = sum(
            1 for analysis in tab_states_analysis if analysis["states_distinguish"]
        )
        visual_changes = sum(
            1 for analysis in tab_states_analysis if analysis["visual_state_changes"]
        )
        css_changes = sum(
            1 for analysis in tab_states_analysis if analysis["css_classes_change"]
        )
        ...
        states_summary = {
            ...
            "states_clearly_different": distinguishable_states
            >= len(tab_states_analysis) * 0.8,
            "visual_feedback_works": visual_changes > 0 or css_changes > 0,
            "all_tabs_provide_feedback": distinguishable_states
            == len(tab_states_analysis),
        }

        tabs_page.log_step(f"Итоги анализа состояний вкладок: {states_summary}")
        allure.attach(
            str(states_summary), "tab_states_summary", allure.attachment_type.JSON
        )

        assert states_summary[
            "visual_feedback_works"
        ], f"Вкладки должны предоставлять визуальную обратную связь: визуальных изменений {visual_changes}, CSS изменений {css_changes}"

        if states_summary["all_tabs_provide_feedback"]:
            tabs_page.log_step(
                "✅ Все вкладки четко различают активное/неактивное состояния"
            )
        elif states_summary["states_clearly_different"]:
            tabs_page.log_step("✅ Большинство вкладок четко различают состояния")
        else:
            tabs_page.log_step(
                "ℹ️ Некоторые вкладки могут не различать состояния визуально"
            )
...
def test_tab_keyboard_navigation(tabs_page: TabsPage):
    ...
    with allure.step("Проверяем доступность вкладок для клавиатурной навигации"):
        ...
        tabs_page.log_step(
            f"Вкладки доступны для клавиатурной навигации: {keyboard_accessible}"
        )
        ...
        with allure.step("Тестируем клавиатурную навигацию по вкладкам"):
            ...
            for i, tab_info in enumerate(all_tabs):
                ...
                with allure.step(f"Клавиатурная навигация к вкладке: '{tab_title}'"):
                    ...
                    tabs_page.log_step(
                        f"Результат клавиатурного теста '{tab_title}': {keyboard_test}"
                    )
        ...
        with allure.step("Тестируем навигацию стрелками"):
            if len(all_tabs) >= 2:
                ...
                arrow_navigation_test = {
                    ...
                    "arrow_navigation_works": second_tab_focused_after_arrow
                    or first_tab_focused_after_back,
                }

                keyboard_navigation_tests.append(
                    {"test_type": "arrow_navigation", **arrow_navigation_test}
                )

                tabs_page.log_step(
                    f"Результат навигации стрелками: {arrow_navigation_test}"
                )
    ...
    with allure.step("Анализируем клавиатурную доступность вкладок"):
        if keyboard_navigation_tests:
            ...
            tab_tests = [
                test for test in keyboard_navigation_tests if "tab_index" in test
            ]

            accessible_tabs = sum(
                1 for test in tab_tests if test["keyboard_accessible"]
            )
            ...
            arrow_tests = [
                test
                for test in keyboard_navigation_tests
                if test.get("test_type") == "arrow_navigation"
            ]
            arrow_navigation_works = any(
                test.get("arrow_navigation_works", False) for test in arrow_tests
            )
            ...
            keyboard_summary = {
                ...
                "full_keyboard_support": accessible_tabs == len(tab_tests)
                and operable_tabs == len(tab_tests),
            }
        ...
        elif keyboard_summary["keyboard_support_available"] and (
            keyboard_summary["accessibility_good"]
            or keyboard_summary["operability_good"]
        ):
...
def test_tabs_full_integration(tabs_page: TabsPage):
    ...
    with allure.step("Выполняем полный цикл работы с вкладками"):
        all_tabs = tabs_page.get_all_tabs_info()
        tabs_page.log_step(
            f"Начинаем интеграционное тестирование {len(all_tabs)} вкладок"
        )
        ...
        final_state_check = {
            ...
            "all_tabs_still_present": len(tabs_page.get_all_tabs_info())
            == len(all_tabs),
            "tabs_still_clickable": all(
                tabs_page.is_tab_clickable(tab["index"]) for tab in all_tabs
            ),
            "content_areas_functional": all(
                tabs_page.is_tab_content_area_functional(tab["index"])
                for tab in all_tabs
            ),
        }
    ...
    with allure.step("Создаем итоговый отчет интеграционного тестирования"):
        ...
        successful_switches = sum(
            1
            for result in integration_results["switching_cycle"]
            if result["switch_successful"]
        )
        content_visibility = sum(
            1
            for result in integration_results["switching_cycle"]
        ...
        fast_responses = sum(
            1
            for result in integration_results["rapid_switching"]
        ...
        integration_summary = {
            ...
            "overall_switching_success_rate": (
                successful_switches / len(all_tabs) if all_tabs else 0
            ),
            "content_display_rate": (
                content_visibility / len(all_tabs) if all_tabs else 0
            ),
            "performance_good": fast_responses
            >= len(integration_results["rapid_switching"]) * 0.8,
            "integration_successful": (
                successful_switches >= len(all_tabs) * 0.8
                and state_stable
                and page_stable
            ),
        }
        ...
        allure.attach(
            ...
            "tabs_integration_summary",
            allure.attachment_type.JSON,
        )

        assert integration_summary[
            "integration_successful"
        ], f"Интеграционное тестирование должно пройти успешно: переключений {successful_switches}/{len(all_tabs)}, состояние стабильно {state_stable}, страница стабильна {page_stable}"
