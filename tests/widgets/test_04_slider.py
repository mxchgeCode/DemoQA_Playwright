...
def test_single_value_slider(slider_page: SliderPage):
    ...
    with allure.step("Проверяем наличие одиночного слайдера"):
        ...
        assert single_slider_present, "Одиночный слайдер должен присутствовать на странице"
    ...
    with allure.step("Получаем начальное значение слайдера"):
        ...
        slider_info = {
            ...
            "value_is_numeric": isinstance(initial_value, (int, float)) or (isinstance(initial_value, str) and initial_value.isdigit())
        }

        allure.attach(str(slider_info), "initial_slider_state", allure.attachment_type.JSON)
    ...
    with allure.step("Получаем границы и свойства слайдера"):
        ...
        allure.attach(str(slider_properties), "slider_properties", allure.attachment_type.JSON)

        # Проверяем что слайдер имеет разумные границы
        min_value = slider_properties.get("min", 0)
        max_value = slider_properties.get("max", 100)
        assert max_value > min_value, f"Максимальное значение должно быть больше минимального: max={max_value}, min={min_value}"
    ...
    with allure.step("Тестируем перемещение ползунка вправо"):
        ...
        right_movement_test = {
            ...
            "value_increased": self._compare_values(value_after_right, initial_value, ">")
        }
    ...
    with allure.step("Тестируем перемещение ползунка влево"):
        ...
        left_movement_test = {
            ...
            "value_decreased": self._compare_values(value_after_left, value_after_right, "<")
        }
    ...
    with allure.step("Тестируем установку конкретного значения"):
        ...
        set_value_test = {
            ...
            "value_matches_target": self._values_approximately_equal(value_after_set, target_value),
            "value_in_expected_range": min_value <= self._to_numeric(value_after_set) <= max_value
        }
    ...
    with allure.step("Анализируем функциональность одиночного слайдера"):
        allure.attach(str(slider_movement_tests), "single_slider_movement_tests", allure.attachment_type.JSON)

        successful_movements = sum(1 for test in slider_movement_tests if test.get("movement_successful") or test.get("set_value_successful"))
        value_changes = sum(1 for test in slider_movement_tests if test.get("value_changed"))
        ...
        slider_page.log_step(f"Итоги тестирования одиночного слайдера: {single_slider_summary}")
        allure.attach(str(single_slider_summary), "single_slider_summary", allure.attachment_type.JSON)

        assert single_slider_summary["slider_responds_to_interaction"], f"Слайдер должен реагировать на взаимодействие: {successful_movements}/{len(slider_movement_tests)}"
        assert single_slider_summary["values_change_on_movement"], f"Значения должны изменяться при движении: {value_changes}/{len(slider_movement_tests)}"

        if single_slider_summary["slider_functional"]:
            slider_page.log_step("✅ Одиночный слайдер полностью функционален")
        else:
            slider_page.log_step("⚠️ Одиночный слайдер имеет ограничения в функциональности")
...
def test_range_slider(slider_page: SliderPage):
    ...
    with allure.step("Проверяем наличие диапазонного слайдера"):
        ...
        slider_page.log_step(f"Диапазонный слайдер присутствует: {range_slider_present}")
    ...
    with allure.step("Получаем начальные значения диапазонного слайдера"):
        ...
        range_info = {
            ...
            "valid_range": self._to_numeric(initial_max_value) >= self._to_numeric(initial_min_value)
        }

        allure.attach(str(range_info), "initial_range_slider_state", allure.attachment_type.JSON)

        assert range_info["valid_range"], f"Максимальное значение должно быть >= минимального: min={initial_min_value}, max={initial_max_value}"

    range_movement_tests = []

    with allure.step("Тестируем перемещение левого ползунка (минимум)"):
        slider_page.log_step("Перемещение левого ползунка для изменения минимального значения")
        ...
        min_handle_test = {
            ...
            "range_still_valid": self._to_numeric(max_value_after_min_move) >= self._to_numeric(min_value_after)
        }
    ...
    with allure.step("Тестируем перемещение правого ползунка (максимум)"):
        slider_page.log_step("Перемещение правого ползунка для изменения максимального значения")
        ...
        max_handle_test = {
            ...
            "range_still_valid": self._to_numeric(max_value_after) >= self._to_numeric(min_value_after_max_move)
        }
    ...
    with allure.step("Тестируем установку конкретного диапазона"):
        ...
        set_range_test = {
            ...
            "min_approximately_correct": self._values_approximately_equal(final_min, target_min, tolerance=10),
            "max_approximately_correct": self._values_approximately_equal(final_max, target_max, tolerance=10),
    ...
    with allure.step("Анализируем функциональность диапазонного слайдера"):
        allure.attach(str(range_movement_tests), "range_slider_movement_tests", allure.attachment_type.JSON)

        successful_movements = sum(1 for test in range_movement_tests if test.get("movement_successful") or test.get("set_range_successful"))
        handle_value_changes = sum(1 for test in range_movement_tests if test.get("value_changed"))
        valid_ranges_maintained = sum(1 for test in range_movement_tests if test.get("range_still_valid", True))
        ...
        range_slider_summary = {
            ...
            "range_slider_functional": successful_movements > 0 and handle_value_changes > 0,
            "range_constraints_respected": valid_ranges_maintained == len(range_movement_tests),
            "movement_details": range_movement_tests
        }

        slider_page.log_step(f"Итоги тестирования диапазонного слайдера: {range_slider_summary}")
        allure.attach(str(range_slider_summary), "range_slider_summary", allure.attachment_type.JSON)

        assert range_slider_summary["range_slider_functional"], f"Диапазонный слайдер должен быть функциональным: движений {successful_movements}, изменений {handle_value_changes}"
        assert range_slider_summary["range_constraints_respected"], f"Ограничения диапазона должны соблюдаться: {valid_ranges_maintained}/{len(range_movement_tests)}"

        if range_slider_summary["range_slider_functional"]:
            slider_page.log_step("✅ Диапазонный слайдер полностью функционален")
        else:
            slider_page.log_step("⚠️ Диапазонный слайдер имеет ограничения в функциональности")
...
def test_slider_precision_and_step(slider_page: SliderPage):
    ...
    with allure.step("Анализируем свойства шага слайдера"):
        ...
        allure.attach(str(slider_step_info), "slider_step_properties", allure.attachment_type.JSON)
        ...
        for test_value in test_values:
            if min_value <= test_value <= max_value:
                with allure.step(f"Установка точного значения: {test_value}"):
                    slider_page.log_step(f"Тестирование установки значения: {test_value}")
                    ...
                    precision_test = {
                        ...
                        "value_exact": self._values_approximately_equal(actual_value, test_value, tolerance=step_value),
                        "value_respects_step": self._value_respects_step(actual_value, min_value, step_value),
                        "value_in_bounds": min_value <= self._to_numeric(actual_value) <= max_value
                    }

                    precision_tests.append(precision_test)
                    slider_page.log_step(f"Результат установки {test_value}: {precision_test}")
    ...
    with allure.step("Тестируем граничные значения"):
        ...
        boundary_tests.append({
            ...
            "correct_boundary": self._values_approximately_equal(min_actual, min_value, tolerance=step_value)
        })
        ...
        boundary_tests.append({
            ...
            "correct_boundary": self._values_approximately_equal(max_actual, max_value, tolerance=step_value)
        })
    ...
    with allure.step("Анализируем точность и соблюдение шага"):
        allure.attach(str(precision_tests), "precision_tests_results", allure.attachment_type.JSON)
        allure.attach(str(boundary_tests), "boundary_tests_results", allure.attachment_type.JSON)

        exact_values = sum(1 for test in precision_tests if test["value_exact"])
        step_compliant = sum(1 for test in precision_tests if test["value_respects_step"])
        in_bounds = sum(1 for test in precision_tests if test["value_in_bounds"])
        correct_boundaries = sum(1 for test in boundary_tests if test["correct_boundary"])
        ...
        precision_summary = {
            ...
            "overall_precision_acceptable": exact_values > 0 and in_bounds == len(precision_tests)
        }

        slider_page.log_step(f"Итоги точности слайдера: {precision_summary}")
        allure.attach(str(precision_summary), "slider_precision_summary", allure.attachment_type.JSON)

        assert precision_summary["overall_precision_acceptable"], f"Общая точность должна быть приемлемой: точных {exact_values}, в границах {in_bounds}/{len(precision_tests)}"
    ...
    def _value_respects_step(self, value, min_value, step):
        ...
        try:
            ...
            return abs(difference % numeric_step) < 0.01 or abs(difference % numeric_step - numeric_step) < 0.01
        except:
