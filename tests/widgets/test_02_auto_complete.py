...
def test_single_auto_complete_functionality(auto_complete_page: AutoCompletePage):
    ...
    with allure.step("Проверяем наличие поля одиночного автодополнения"):
        single_input_present = auto_complete_page.is_single_auto_complete_input_present()
        auto_complete_page.log_step(f"Поле одиночного автодополнения присутствует: {single_input_present}")

        assert single_input_present, "Поле одиночного автодополнения должно присутствовать на странице"
    ...
    with allure.step("Очищаем поле и проверяем начальное состояние"):
        ...
        assert len(initial_value) == 0, f"Поле должно быть пустым изначально: '{initial_value}'"

    autocomplete_results = []

    for test_input in test_inputs:
        with allure.step(f"Тестируем автодополнение для ввода: '{test_input}'"):
            auto_complete_page.log_step(f"Ввод текста для автодополнения: '{test_input}'")
            ...
            autocomplete_test = {
                ...
                "autocomplete_triggered": suggestions_visible and len(suggestions_list) > 0
            }

            autocomplete_results.append(autocomplete_test)
            auto_complete_page.log_step(f"Результат автодополнения для '{test_input}': {autocomplete_test}")

            # Если есть предложения, пытаемся выбрать первое
            if suggestions_visible and len(suggestions_list) > 0:
                auto_complete_page.log_step(f"Выбор первого предложения: '{suggestions_list[0]}'")
                ...
                if selection_result:
                    ...
                    auto_complete_page.log_step(f"Значение после выбора: '{final_value}'")
                else:
                    autocomplete_test["first_suggestion_selected"] = False

    with allure.step("Анализируем результаты одиночного автодополнения"):
        allure.attach(str(autocomplete_results), "single_autocomplete_results", allure.attachment_type.JSON)

        successful_autocompletes = sum(1 for result in autocomplete_results if result["autocomplete_triggered"])
        successful_selections = sum(1 for result in autocomplete_results if result.get("first_suggestion_selected", False))
        ...
        single_autocomplete_summary = {
            ...
            "autocomplete_success_rate": successful_autocompletes / len(autocomplete_results) if autocomplete_results else 0,
            "selection_success_rate": successful_selections / len(autocomplete_results) if autocomplete_results else 0,
            "autocomplete_works": successful_autocompletes > 0,
            "selection_works": successful_selections > 0
        }

        auto_complete_page.log_step(f"Итоги одиночного автодополнения: {single_autocomplete_summary}")
        allure.attach(str(single_autocomplete_summary), "single_autocomplete_summary", allure.attachment_type.JSON)

        assert single_autocomplete_summary["autocomplete_works"], f"Автодополнение должно работать хотя бы для одного ввода: {successful_autocompletes}/{len(autocomplete_results)}"

        if single_autocomplete_summary["selection_works"]:
            auto_complete_page.log_step("✅ Выбор предложений одиночного автодополнения работает")
        else:
            auto_complete_page.log_step("ℹ️ Выбор предложений требует дополнительной проверки")
...
def test_multiple_auto_complete_functionality(auto_complete_page: AutoCompletePage):
    ...
    with allure.step("Проверяем наличие поля множественного автодополнения"):
        multiple_input_present = auto_complete_page.is_multiple_auto_complete_input_present()
        auto_complete_page.log_step(f"Поле множественного автодополнения присутствует: {multiple_input_present}")

        assert multiple_input_present, "Поле множественного автодополнения должно присутствовать на странице"
    ...
    with allure.step("Очищаем поле множественного автодополнения"):
        ...
        assert len(initial_values) == 0, f"Поле должно быть пустым изначально: {initial_values}"
    ...
    for i, color in enumerate(test_colors):
        with allure.step(f"Добавляем цвет {i + 1}: '{color}'"):
            ...
            multiple_suggestions_visible = auto_complete_page.are_multiple_suggestions_visible()
            ...
            if color_found_in_suggestions:
                selection_success = auto_complete_page.select_multiple_suggestion_by_text(color)
            else:
                # Пытаемся выбрать первое предложение
                selection_success = auto_complete_page.select_first_multiple_suggestion()
            ...
            auto_complete_page.log_step(f"Результат добавления '{color}': {selection_test}")
    ...
    with allure.step("Тестируем удаление выбранных значений"):
        current_values_before_removal = auto_complete_page.get_multiple_selected_values()
        auto_complete_page.log_step(f"Значения перед удалением: {current_values_before_removal}")
        ...
        if len(current_values_before_removal) > 0:
            ...
            auto_complete_page.log_step(f"Попытка удаления первого значения: '{first_value_to_remove}'")
            ...
            removal_test = {
                ...
                "value_actually_removed": first_value_to_remove not in values_after_removal,
                "count_decreased": len(values_after_removal) < len(current_values_before_removal)
            }
    ...
    with allure.step("Анализируем результаты множественного автодополнения"):
        allure.attach(str(multiple_selections), "multiple_autocomplete_selections", allure.attachment_type.JSON)
        allure.attach(str(removal_tests), "multiple_value_removals", allure.attachment_type.JSON)

        successful_additions = sum(1 for selection in multiple_selections if selection["value_added"])
        suggestions_appeared_count = sum(1 for selection in multiple_selections if selection["suggestions_appeared"])
        successful_removals = sum(1 for removal in removal_tests if removal["value_actually_removed"])
        ...
        multiple_autocomplete_summary = {
            ...
            "removal_functionality_works": successful_removals > 0 or len(removal_tests) == 0
        }

        auto_complete_page.log_step(f"Итоги множественного автодополнения: {multiple_autocomplete_summary}")
        allure.attach(str(multiple_autocomplete_summary), "multiple_autocomplete_summary", allure.attachment_type.JSON)

        assert multiple_autocomplete_summary["autocomplete_suggestions_work"], f"Должны появляться предложения автодополнения: {suggestions_appeared_count}/{len(multiple_selections)}"
        assert multiple_autocomplete_summary["successful_additions"] > 0, f"Должно быть добавлено хотя бы одно значение: {successful_additions}/{len(multiple_selections)}"

        if multiple_autocomplete_summary["multiple_selection_works"]:
            auto_complete_page.log_step("✅ Множественное автодополнение работает корректно")
        else:
            auto_complete_page.log_step("ℹ️ Удалось добавить только одно значение - проверьте настройки множественного выбора")
