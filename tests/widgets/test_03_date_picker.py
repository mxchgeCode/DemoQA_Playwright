...
def test_simple_date_picker_selection(date_picker_page: DatePickerPage):
    ...
    with allure.step("Проверяем наличие поля выбора даты"):
        ...
        date_picker_page.log_step(
            f"Поле выбора даты присутствует: {date_input_present}"
        )
    ...
    with allure.step("Получаем текущее значение поля даты"):
        ...
        allure.attach(
            initial_date_value, "initial_date_value", allure.attachment_type.TEXT
        )
    ...
    with allure.step("Анализируем структуру календаря"):
        ...
        allure.attach(
            str(calendar_info), "calendar_structure_info", allure.attachment_type.JSON
        )

        # Проверяем основные элементы календаря
        assert calendar_info.get(
            "has_navigation", False
        ), "Календарь должен иметь навигацию по месяцам"
        assert calendar_info.get(
            "has_date_cells", False
        ), "Календарь должен содержать ячейки с датами"
    ...
    with allure.step("Выбираем дату в календаре"):
        ...
        assert (
            date_selection_result
        ), "Должна быть выбрана хотя бы одна дата в календаре"
    ...
    with allure.step("Проверяем результат выбора даты"):
        ...
        allure.attach(
            str(date_selection_summary),
            "date_selection_summary",
            allure.attachment_type.JSON,
        )

        # Проверяем что дата была выбрана
        assert date_selection_summary[
            "final_date_not_empty"
        ], f"Поле даты не должно быть пустым после выбора: '{final_date_value}'"

        if date_selection_summary["date_changed"]:
            date_picker_page.log_step("✅ Дата успешно изменена через календарь")
        else:
            date_picker_page.log_step(
                "ℹ️ Возможно, была выбрана та же дата или дата не изменилась"
            )

        if date_selection_summary["calendar_closed_after_selection"]:
            date_picker_page.log_step(
                "✅ Календарь автоматически закрылся после выбора"
            )
...
def test_manual_date_input(date_picker_page: DatePickerPage):
    ...
    test_dates = ["12/31/2023", "01/15/2024", "06/20/2024", "12/25/2024"]
    ...
    with allure.step("Тестируем ручной ввод различных дат"):
        for test_date in test_dates:
            with allure.step(f"Ввод даты: {test_date}"):
                date_picker_page.log_step(
                    f"Тестирование ручного ввода даты: {test_date}"
                )
                ...
                manual_input_test = {
                    ...
                    "format_accepted": resulting_value == test_date
                    or (date_accepted and not has_validation_error),
                }

                manual_input_results.append(manual_input_test)
                date_picker_page.log_step(
                    f"Результат ввода '{test_date}': {manual_input_test}"
                )
    ...
    with allure.step("Тестируем некорректные форматы дат"):
        ...
        for invalid_date in invalid_dates:
            with allure.step(f"Тест некорректной даты: {invalid_date}"):
                date_picker_page.log_step(
                    f"Тестирование некорректной даты: {invalid_date}"
                )
                ...
                invalid_test = {
                    ...
                    "resulting_value": resulting_value,
                    "validation_message": validation_message,
                    "properly_rejected": len(validation_message) > 0
                    or resulting_value == ""
                    or resulting_value != invalid_date,
                }

                manual_input_results.append(invalid_test)
                date_picker_page.log_step(
                    f"Результат некорректного ввода '{invalid_date}': {invalid_test}"
                )

    with allure.step("Анализируем результаты ручного ввода дат"):
        allure.attach(
            str(manual_input_results),
            "manual_date_input_results",
            allure.attachment_type.JSON,
        )
        ...
        properly_rejected_invalid = sum(
            1 for r in invalid_inputs if r["properly_rejected"]
        )
        ...
        manual_input_summary = {
            ...
            "valid_date_acceptance_rate": (
                accepted_valid_dates / len(valid_inputs) if valid_inputs else 0
            ),
            "invalid_date_rejection_rate": (
                properly_rejected_invalid / len(invalid_inputs) if invalid_inputs else 0
            ),
            "manual_input_works": accepted_valid_dates > 0,
            "validation_works": properly_rejected_invalid > 0
            or len(invalid_inputs) == 0,
        }

        date_picker_page.log_step(f"Итоги ручного ввода дат: {manual_input_summary}")
        allure.attach(
            str(manual_input_summary),
            "manual_input_summary",
            allure.attachment_type.JSON,
        )

        assert manual_input_summary[
            "manual_input_works"
        ], f"Ручной ввод должен работать: принято {accepted_valid_dates}/{len(valid_inputs)} валидных дат"

        if manual_input_summary["validation_works"]:
            date_picker_page.log_step("✅ Валидация дат работает корректно")
        else:
            date_picker_page.log_step(
                "ℹ️ Валидация дат может требовать дополнительной настройки"
            )
...
def test_date_and_time_picker(date_picker_page: DatePickerPage):
    ...
    with allure.step("Проверяем наличие поля выбора даты и времени"):
        ...
        date_picker_page.log_step(
            f"Поле выбора даты и времени присутствует: {datetime_input_present}"
        )

        if not datetime_input_present:
            date_picker_page.log_step(
                "⚠️ Поле даты и времени не найдено, используем обычное поле даты"
            )
    ...
    with allure.step("Получаем начальное значение даты и времени"):
        ...
        date_picker_page.log_step(
            f"Начальное значение даты и времени: '{initial_datetime_value}'"
        )
    ...
    with allure.step("Открываем календарь для выбора даты и времени"):
        ...
        if datetime_calendar_opened:
            ...
            date_picker_page.log_step(
                f"Информация о календаре даты-времени: {calendar_info}"
            )

            allure.attach(
                str(calendar_info),
                "datetime_calendar_info",
                allure.attachment_type.JSON,
            )
        ...
        if datetime_calendar_opened and date_selected:
            ...
            if time_picker_available:
                ...
                if not time_set_result:
                    ...
                    date_picker_page.log_step(
                        f"Выбрано предустановленное время: {time_selected}"
                    )
            else:
    ...
    with allure.step("Подтверждаем выбор даты и времени"):
        if datetime_calendar_opened:
            ...
            datetime_selection_result = {
                ...
                "appears_to_include_time": ":"
                in final_datetime_value,  # Проверяем наличие времени
            }

            date_picker_page.log_step(
                f"Результат выбора даты и времени: {datetime_selection_result}"
            )
            allure.attach(
                str(datetime_selection_result),
                "datetime_selection_result",
                allure.attachment_type.JSON,
            )

            assert datetime_selection_result[
                "has_datetime_value"
            ], f"Поле должно содержать значение после выбора: '{final_datetime_value}'"
...
def test_date_range_selection(date_picker_page: DatePickerPage):
    ...
    with allure.step("Проверяем наличие полей для выбора диапазона дат"):
        ...
        date_picker_page.log_step(
            f"Выбор диапазона дат доступен: {date_range_available}"
        )

        if not date_range_available:
            date_picker_page.log_step(
                "ℹ️ Функциональность диапазона дат недоступна на данной странице"
            )
            return
    ...
    with allure.step("Анализируем результат выбора диапазона"):
        ...
        allure.attach(
            str(final_range_state),
            "date_range_selection_result",
            allure.attachment_type.JSON,
        )
...
def test_calendar_navigation(date_picker_page: DatePickerPage):
    ...
    with allure.step("Открываем календарь для тестирования навигации"):
        ...
        if not calendar_opened:
            date_picker_page.log_step(
                "⚠️ Не удалось открыть календарь для тестирования навигации"
            )
            return

        assert (
            calendar_opened
        ), "Календарь должен открываться для тестирования навигации"
    ...
    with allure.step("Получаем текущий месяц и год в календаре"):
        ...
        allure.attach(
            str(current_month_year),
            "initial_calendar_month_year",
            allure.attachment_type.JSON,
        )
    ...
    with allure.step("Тестируем навигацию на следующий месяц"):
        ...
        date_picker_page.log_step(
            f"Результат навигации на следующий месяц: {next_month_test}"
        )
    ...
    with allure.step("Тестируем навигацию на предыдущий месяц"):
        ...
        date_picker_page.log_step(
            f"Результат навигации на предыдущий месяц: {prev_month_test}"
        )
    ...
    with allure.step("Тестируем быструю навигацию по годам"):
        ...
        if year_navigation_available:
            ...
            date_picker_page.log_step(
                f"Результат навигации по годам: {year_navigation_test}"
            )
        else:
    ...
    with allure.step("Анализируем результаты навигации по календарю"):
        allure.attach(
            str(navigation_tests),
            "calendar_navigation_tests",
            allure.attachment_type.JSON,
        )

        successful_navigations = sum(
            1
            for _, test in navigation_tests
            if test.get("navigation_successful", False)
        )
        navigations_with_change = sum(
            1
            for _, test in navigation_tests
            if test.get("month_changed", False) or test.get("year_changed", False)
        )
        ...
        navigation_summary = {
            ...
            "navigation_success_rate": (
                successful_navigations / len(navigation_tests)
                if navigation_tests
                else 0
            ),
            "calendar_navigation_works": successful_navigations > 0,
            "month_navigation_works": any(
                test[0] in ["next_month", "previous_month"]
                and test[1].get("month_changed", False)
        ...
        allure.attach(
            ...
            "calendar_navigation_summary",
            allure.attachment_type.JSON,
        )

        assert navigation_summary[
            "calendar_navigation_works"
        ], f"Навигация по календарю должна работать: {successful_navigations}/{len(navigation_tests)} успешных"

        if navigation_summary["month_navigation_works"]:
            date_picker_page.log_step("✅ Навигация по месяцам работает корректно")
        else:
            date_picker_page.log_step(
                "⚠️ Навигация по месяцам может требовать дополнительной настройки"
            )
