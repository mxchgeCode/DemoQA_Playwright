...
def test_basic_tooltip_hover(tool_tips_page: ToolTipsPage):
    ...
    with allure.step("Проверяем наличие элементов с подсказками"):
        ...
        tool_tips_page.log_step(
            f"Найдено элементов с подсказками: {len(elements_with_tooltips)}"
        )
        ...
        allure.attach(
            str(tooltip_elements_info),
            "tooltip_elements_info",
            allure.attachment_type.JSON,
        )

        assert (
            len(elements_with_tooltips) > 0
        ), "Должен быть хотя бы один элемент с подсказкой"

    tooltip_hover_tests = []

    with allure.step("Тестируем отображение подсказок при наведении"):
        for i, element_info in enumerate(
            elements_with_tooltips[:4]
        ):  # Тестируем максимум 4 элемента
            ...
            with allure.step(f"Тест подсказки для элемента: {element_type}"):
                ...
                hover_test = {
                    ...
                    "text_matches": (
                        tooltip_text == expected_tooltip_text
                        if expected_tooltip_text
                        else len(tooltip_text) > 0
                    ),
                    "tooltip_position": tooltip_position,
                    "tooltip_positioned": tooltip_position.get("x", 0) > 0
                    and tooltip_position.get("y", 0) > 0,
                }
                ...
                if (
                    hover_test["tooltip_appeared"]
                    and hover_test["tooltip_hidden_after_mouse_away"]
                ):
                    tool_tips_page.log_step(
                        f"✅ Подсказка для {element_type} работает корректно"
                    )
                elif hover_test["tooltip_appeared"]:
                    tool_tips_page.log_step(
                        f"ℹ️ Подсказка для {element_type} появляется, но может не скрываться"
                    )
                else:
                    tool_tips_page.log_step(
                        f"⚠️ Подсказка для {element_type} не появляется"
                    )

    with allure.step("Анализируем функциональность подсказок"):
        allure.attach(
            str(tooltip_hover_tests), "tooltip_hover_tests", allure.attachment_type.JSON
        )

        tooltips_appeared = sum(
            1 for test in tooltip_hover_tests if test["tooltip_appeared"]
        )
        tooltips_hidden = sum(
            1
            for test in tooltip_hover_tests
            if test.get("tooltip_hidden_after_mouse_away", False)
        )
        tooltips_with_text = sum(
            1 for test in tooltip_hover_tests if len(test["tooltip_text"]) > 0
        )
        tooltips_positioned = sum(
            1 for test in tooltip_hover_tests if test["tooltip_positioned"]
        )
        ...
        tooltip_summary = {
            ...
            "tooltip_appearance_rate": (
                tooltips_appeared / len(tooltip_hover_tests)
                if tooltip_hover_tests
                else 0
            ),
            "tooltip_hiding_rate": (
                tooltips_hidden / len(tooltip_hover_tests) if tooltip_hover_tests else 0
            ),
            "tooltips_functional": tooltips_appeared > 0 and tooltips_with_text > 0,
            "tooltip_behavior_good": tooltips_appeared
            >= len(tooltip_hover_tests) * 0.8,
        }

        tool_tips_page.log_step(f"Итоги тестирования подсказок: {tooltip_summary}")
        allure.attach(
            str(tooltip_summary),
            "tooltip_functionality_summary",
            allure.attachment_type.JSON,
        )

        assert tooltip_summary[
            "tooltips_functional"
        ], f"Подсказки должны быть функциональными: появилось {tooltips_appeared}, с текстом {tooltips_with_text}"
...
def test_tooltip_positioning(tool_tips_page: ToolTipsPage):
        ...
        for i, element_info in enumerate(
            ...
            with allure.step(f"Анализ позиционирования для: {element_type}"):
                ...
                if tooltip_visible:
                    ...
                    relative_position = (
                        tool_tips_page.analyze_tooltip_relative_position(
                            element_position,
                            element_size,
                            tooltip_position,
                            tooltip_size,
                    ...
                    within_viewport = tool_tips_page.is_tooltip_within_viewport(
                        tooltip_position, tooltip_size
                    )
                ...
                tool_tips_page.log_step(
                    f"Результат позиционирования {i}: {positioning_test}"
                )
    ...
    with allure.step("Анализируем качество позиционирования"):
        allure.attach(
            str(positioning_tests),
            "tooltip_positioning_tests",
            allure.attachment_type.JSON,
        )

        visible_tooltips = sum(
            1 for test in positioning_tests if test["tooltip_visible"]
        )
        good_positioning = sum(
            1 for test in positioning_tests if test.get("positioning_good", False)
        )
        within_viewport = sum(
            1 for test in positioning_tests if test.get("within_viewport", False)
        )
        no_overlaps = sum(
            1 for test in positioning_tests if not test.get("overlaps_element", True)
        )
        ...
        positioning_summary = {
            ...
            "positioning_success_rate": (
                good_positioning / visible_tooltips if visible_tooltips > 0 else 0
            ),
            "viewport_compliance_rate": (
                within_viewport / visible_tooltips if visible_tooltips > 0 else 0
            ),
            "positioning_excellent": good_positioning == visible_tooltips
            and visible_tooltips > 0,
        }

        tool_tips_page.log_step(f"Итоги позиционирования: {positioning_summary}")
        allure.attach(
            str(positioning_summary), "positioning_summary", allure.attachment_type.JSON
        )
        ...
        if positioning_summary["positioning_excellent"]:
            ...
            tool_tips_page.log_step(
                "ℹ️ Позиционирование подсказок может требовать улучшения"
            )
...
def test_tooltip_timing(tool_tips_page: ToolTipsPage):
        ...
        for i, element_info in enumerate(
            ...
            with allure.step(f"Измерение тайминга для: {element_type}"):
                ...
                timing_test = {
                    ...
                    "appearance_fast": appearance_time
                    and appearance_time <= 1.0,  # Менее 1 секунды
                    "disappearance_fast": disappearance_time
                    and disappearance_time <= 1.0,
                    "timing_responsive": (appearance_time and appearance_time <= 1.5)
                    and (disappearance_time is None or disappearance_time <= 1.5),
                }
    ...
    with allure.step("Анализируем производительность тайминга"):
        allure.attach(
            str(timing_tests), "tooltip_timing_tests", allure.attachment_type.JSON
        )

        tooltips_appeared = sum(1 for test in timing_tests if test["tooltip_appeared"])
        tooltips_disappeared = sum(
            1 for test in timing_tests if test["tooltip_disappeared"]
        )
        fast_appearance = sum(
            1 for test in timing_tests if test.get("appearance_fast", False)
        )
        fast_disappearance = sum(
            1 for test in timing_tests if test.get("disappearance_fast", False)
        )
        responsive_timing = sum(
            1 for test in timing_tests if test.get("timing_responsive", False)
        )

        if tooltips_appeared > 0:
            avg_appearance_time = (
                sum(
                    test["appearance_time_seconds"]
                    for test in timing_tests
                    if test["appearance_time_seconds"]
                )
                / tooltips_appeared
            )
        else:
        ...
        if tooltips_disappeared > 0:
            avg_disappearance_time = (
                sum(
                    test["disappearance_time_seconds"]
                    for test in timing_tests
                    if test["disappearance_time_seconds"]
                )
                / tooltips_disappeared
            )
        else:
        ...
        timing_summary = {
            ...
            "appearance_performance_good": (
                fast_appearance >= tooltips_appeared * 0.8
                if tooltips_appeared > 0
                else False
            ),
        }
...
def test_different_tooltip_types(tool_tips_page: ToolTipsPage):
    ...
    with allure.step("Анализируем типы подсказок"):
        ...
        allure.attach(
            str(tooltip_types_info),
            "tooltip_types_analysis",
            allure.attachment_type.JSON,
        )
    ...
    if tooltip_types_info.get("button_tooltip_available", False):
        with allure.step("Тестируем подсказку кнопки"):
            ...
            tool_tips_page.log_step(
                f"Результат подсказки кнопки: {button_tooltip_test}"
            )
    ...
    if tooltip_types_info.get("text_field_tooltip_available", False):
        with allure.step("Тестируем подсказку текстового поля"):
            ...
            tool_tips_page.log_step(
                f"Результат подсказки текстового поля: {text_field_tooltip_test}"
            )
    ...
    if tooltip_types_info.get("contrary_tooltip_available", False):
        with allure.step("Тестируем противоположную подсказку"):
            ...
            tool_tips_page.log_step(
                f"Результат противоположной подсказки: {contrary_tooltip_test}"
            )
    ...
    with allure.step("Анализируем разнообразие типов подсказок"):
        allure.attach(
            str(type_tests), "tooltip_types_tests", allure.attachment_type.JSON
        )

        if type_tests:
            successful_types = sum(
                1 for test in type_tests if test.get("tooltip_works", False)
            )
            different_behaviors = len(
                set(test.get("behavior_type", "default") for test in type_tests)
            )
            different_positions = len(
                set(test.get("position_type", "default") for test in type_tests)
            )
        ...
        allure.attach(
            str(types_summary), "tooltip_types_summary", allure.attachment_type.JSON
        )
        ...
        if types_summary["all_types_work"] and types_summary["total_types_tested"] > 0:
            ...
            tool_tips_page.log_step(
                "ℹ️ Специфические типы подсказок не найдены или не работают"
            )
        ...
        for i, element_info in enumerate(elements_with_tooltips[:3]):  # Первые 3
            ...
            with allure.step(f"Проверка доступности для: {element_type}"):
                ...
                if keyboard_focusable:
                    ...
                    tooltip_disappears_on_blur = not tool_tips_page.is_tooltip_visible(
                        i
                    )
                else:
                    focus_result = False
                ...
                accessibility_test = {
                    ...
                    "tooltip_contrast_good": tooltip_contrast.get(
                        "contrast_good", False
                    ),
                    ...
                    "accessibility_good": (
                        keyboard_focusable
                        and tooltip_appears_on_focus
                        and tooltip_contrast.get("contrast_good", False)
                    ),
                }

                accessibility_tests.append(accessibility_test)
                tool_tips_page.log_step(
                    f"Результат доступности {i}: {accessibility_test}"
                )

    with allure.step("Анализируем общую доступность подсказок"):
        allure.attach(
            str(accessibility_tests),
            "tooltip_accessibility_tests",
            allure.attachment_type.JSON,
        )

        keyboard_accessible = sum(
            1 for test in accessibility_tests if test["keyboard_focusable"]
        )
        tooltips_on_focus = sum(
            1 for test in accessibility_tests if test["tooltip_on_focus"]
        )
        good_contrast = sum(
            1 for test in accessibility_tests if test["tooltip_contrast_good"]
        )
        aria_support = sum(
            1
            for test in accessibility_tests
            if test["has_aria_describedby"] or test["has_title_attribute"]
        )
        fully_accessible = sum(
            1 for test in accessibility_tests if test["accessibility_good"]
        )
        ...
        accessibility_summary = {
            ...
            "keyboard_accessibility_rate": (
                keyboard_accessible / len(accessibility_tests)
                if accessibility_tests
                else 0
            ),
            "focus_functionality_rate": (
                tooltips_on_focus / keyboard_accessible
                if keyboard_accessible > 0
                else 0
            ),
            "accessibility_compliance_good": fully_accessible
            >= len(accessibility_tests) * 0.6,
        }
        ...
        if accessibility_summary["accessibility_compliance_good"]:
            tool_tips_page.log_step(
                "✅ Доступность подсказок соответствует хорошим стандартам"
            )
        ...
        assert (
            len(accessibility_tests) > 0
        ), "Должны быть проверены элементы на доступность"
