...
def test_single_accordion_section_expand(accordion_page: AccordionPage):
    ...
    with allure.step("Проверяем начальное состояние всех секций аккордеона"):
        ...
        allure.attach(str(initial_states), "initial_accordion_states", allure.attachment_type.JSON)

        assert len(initial_states) > 0, "Должна быть хотя бы одна секция аккордеона"

        # Проверяем что есть как минимум 3 секции
        assert len(initial_states) >= 3, f"Должно быть минимум 3 секции, найдено: {len(initial_states)}"

    with allure.step("Раскрываем первую секцию аккордеона"):
        first_section_initially_open = initial_states[0]["is_expanded"]
        accordion_page.log_step(f"Первая секция изначально открыта: {first_section_initially_open}")
        ...
        if not first_section_initially_open:
            ...
            accordion_page.log_step(f"Результат клика по первой секции: {expand_result}")
    ...
    with allure.step("Проверяем состояние после раскрытия первой секции"):
        ...
        accordion_page.log_step(f"Состояния после раскрытия первой секции: {states_after_first}")
        ...
        accordion_page.log_step(f"Содержимое первой секции: {first_section_content[:100]}...")
        ...
        allure.attach(str(first_section_result), "first_section_expansion_result", allure.attachment_type.JSON)
    ...
    with allure.step("Раскрываем вторую секцию"):
        ...
        accordion_page.log_step(f"Состояния после раскрытия второй секции: {states_after_second}")
        ...
        allure.attach(str(accordion_behavior), "accordion_behavior_analysis", allure.attachment_type.JSON)
    ...
    with allure.step("Тестируем закрытие открытой секции"):
        ...
        allure.attach(str(close_test_result), "section_close_test_result", allure.attachment_type.JSON)

        if close_test_result["section_toggles"]:
            accordion_page.log_step("✅ Секции корректно переключаются (toggle)")
        else:
            accordion_page.log_step("ℹ️ Секция остается открытой - возможно режим 'always expanded'")
    ...
    with allure.step("Последовательно открываем все секции"):
        ...
        for i in range(min(3, len(all_states))):  # Тестируем максимум 3 секции
            with allure.step(f"Открытие секции {i + 1}"):
                ...
                open_sections_count = sum(1 for section in all_states_after if section["is_expanded"])
                ...
                section_test = {
                    ...
                    "other_sections_states": [s["is_expanded"] for j, s in enumerate(all_states_after) if j != i]
                }

                sections_to_test.append(section_test)
                accordion_page.log_step(f"Результат открытия секции {i}: {section_test}")

    with allure.step("Анализируем поведение множественных секций"):
        allure.attach(str(sections_to_test), "multiple_sections_test_results", allure.attachment_type.JSON)

        successfully_opened = sum(1 for test in sections_to_test if test["section_opened"])
        max_open_simultaneously = max(test["total_open_sections"] for test in sections_to_test)
        ...
        accordion_page.log_step(f"Анализ множественных секций: {multiple_sections_analysis}")
        allure.attach(str(multiple_sections_analysis), "multiple_sections_analysis", allure.attachment_type.JSON)

        # Проверяем что секции открываются
        assert multiple_sections_analysis["successfully_opened"] > 0, f"Хотя бы одна секция должна открыться: {successfully_opened}/{len(sections_to_test)}"

        if multiple_sections_analysis["supports_multiple_open"]:
            accordion_page.log_step("✅ Аккордеон поддерживает множественное раскрытие секций")
        else:
...
def test_accordion_sections_content(accordion_page: AccordionPage):
    ...
    with allure.step("Анализируем содержимое всех секций"):
        ...
        for i, section_info in enumerate(all_sections):
            with allure.step(f"Анализ содержимого секции {i + 1}: '{section_info['header_text']}'"):
                ...
                content_element_count = accordion_page.count_elements_in_section_content(i)

                content_analysis = {
                    "section_index": i,
                    "header_text": section_info["header_text"],
                    "content_text": section_content[:200] + "..." if len(section_content) > 200 else section_content,
    ...
    with allure.step("Проверяем уникальность содержимого секций"):
        allure.attach(str(sections_content), "all_sections_content_analysis", allure.attachment_type.JSON)
        ...
        meaningful_content_count = sum(1 for section in sections_content if section["has_meaningful_content"])
        visible_content_count = sum(1 for section in sections_content if section["content_visible"])
        ...
        content_uniqueness_analysis = {
            ...
            "most_sections_have_content": meaningful_content_count >= len(sections_content) * 0.5
        }

        accordion_page.log_step(f"Анализ уникальности содержимого: {content_uniqueness_analysis}")
        ...
        assert content_uniqueness_analysis["all_headers_unique"], f"Все заголовки должны быть уникальными: {len(unique_headers)}/{len(headers)}"
        assert content_uniqueness_analysis["most_sections_have_content"], f"Большинство секций должны содержать значимый контент: {meaningful_content_count}/{len(sections_content)}"

        if content_uniqueness_analysis["all_contents_unique"]:
            accordion_page.log_step("✅ Все секции содержат уникальный контент")
        else:
            accordion_page.log_step("ℹ️ Некоторые секции могут содержать повторяющийся контент")
...
def test_accordion_keyboard_navigation(accordion_page: AccordionPage):
    ...
    with allure.step("Проверяем доступность аккордеона для клавиатурной навигации"):
        ...
        accordion_page.log_step(f"Аккордеон доступен для клавиатурной навигации: {accordion_focusable}")
        ...
        allure.attach(str(accessibility_info), "accordion_accessibility_info", allure.attachment_type.JSON)
        ...
        with allure.step("Тестируем навигацию клавишами"):
            ...
            for i in range(min(3, sections_count)):
                with allure.step(f"Тест клавиатурной навигации к секции {i + 1}"):
                    ...
                    accordion_page.log_step(f"Результат клавиатурного теста секции {i}: {keyboard_test}")
    ...
    with allure.step("Анализируем клавиатурную доступность"):
        if keyboard_navigation_tests:
            ...
            accessible_sections = sum(1 for test in keyboard_navigation_tests if test["keyboard_accessible"])
            operable_sections = sum(1 for test in keyboard_navigation_tests if test["keyboard_operable"])
            ...
            keyboard_accessibility_summary = {
                ...
                "accessibility_rate": accessible_sections / len(keyboard_navigation_tests) if keyboard_navigation_tests else 0,
                "operability_rate": operable_sections / len(keyboard_navigation_tests) if keyboard_navigation_tests else 0,
                "keyboard_support_good": accessible_sections >= len(keyboard_navigation_tests) * 0.5
            }
        ...
        accordion_page.log_step(f"Итоги клавиатурной доступности: {keyboard_accessibility_summary}")
        allure.attach(str(keyboard_accessibility_summary), "keyboard_accessibility_summary", allure.attachment_type.JSON)

        if keyboard_accessibility_summary["keyboard_support_good"]:
            accordion_page.log_step("✅ Аккордеон хорошо поддерживает клавиатурную навигацию")
        elif accordion_focusable:
            accordion_page.log_step("ℹ️ Аккордеон частично поддерживает клавиатурную навигацию")
        else:
            accordion_page.log_step("⚠️ Клавиатурная навигация недоступна или ограничена")
    ...
    with allure.step("Тестируем производительность анимаций аккордеона"):
        ...
        for i in range(min(2, len(sections_info))):  # Тестируем первые 2 секции
            with allure.step(f"Тест анимации секции {i + 1}"):
                ...
                expansion_complete = accordion_page.wait_for_section_expansion(i, timeout=3000)
                ...
                collapse_complete = accordion_page.wait_for_section_collapse(i, timeout=3000)
                ...
                animation_test = {
                    ...
                    "expansion_reasonable_speed": 100 <= expansion_duration <= 2000,  # 0.1-2 секунды
                ...
                accordion_page.log_step(f"Результат теста анимации секции {i}: {animation_test}")

    with allure.step("Анализируем производительность анимаций"):
        allure.attach(str(animation_tests), "animation_performance_tests", allure.attachment_type.JSON)

        if animation_tests:
            avg_expansion_time = sum(test["expansion_duration_ms"] for test in animation_tests) / len(animation_tests)
            avg_collapse_time = sum(test["collapse_duration_ms"] for test in animation_tests) / len(animation_tests)

            smooth_animations = sum(1 for test in animation_tests if test["animation_smooth"])
            reasonable_speed_animations = sum(1 for test in animation_tests if test["expansion_reasonable_speed"] and test["collapse_reasonable_speed"])
            ...
            performance_summary = {
                ...
                "performance_good": reasonable_speed_animations >= len(animation_tests) * 0.5,
                "animations_smooth": smooth_animations >= len(animation_tests) * 0.5,
                "overall_performance_acceptable": reasonable_speed_animations > 0 and avg_expansion_time < 3000
            }
        ...
        accordion_page.log_step(f"Итоги производительности анимаций: {performance_summary}")
        allure.attach(str(performance_summary), "animation_performance_summary", allure.attachment_type.JSON)

        if performance_summary["overall_performance_acceptable"]:
            accordion_page.log_step("✅ Производительность анимаций аккордеона приемлемая")
        else:
            accordion_page.log_step("⚠️ Производительность анимаций требует оптимизации")

        # Базовая проверка что анимации работают
        assert len(animation_tests) > 0, "Должен быть проведен хотя бы один тест анимации"
