...
def test_static_progress_bar_display(progress_bar_page: ProgressBarPage):
    ...
    with allure.step("Проверяем наличие статического прогресс-бара"):
        ...
        progress_bar_page.log_step(
            f"Статический прогресс-бар присутствует: {static_progress_present}"
        )

        assert (
            static_progress_present
        ), "Статический прогресс-бар должен присутствовать на странице"
    ...
    with allure.step("Получаем свойства статического прогресс-бара"):
        ...
        static_progress_info = {
            ...
            "has_numeric_value": isinstance(progress_value, (int, float))
            or (
                isinstance(progress_value, str)
                and progress_value.replace(".", "").isdigit()
            ),
            "percentage_valid": (
        ...
        allure.attach(
            str(static_progress_info),
            "static_progress_bar_info",
            allure.attachment_type.JSON,
        )

        # Основные проверки статического прогресс-бара
        assert static_progress_info[
            "has_numeric_value"
        ], f"Прогресс-бар должен иметь числовое значение: {progress_value}"

        if static_progress_info["percentage_valid"]:
            progress_bar_page.log_step(
                "✅ Процентное значение в валидном диапазоне (0-100%)"
            )
        else:
            progress_bar_page.log_step(
                "ℹ️ Процентное значение может быть в нестандартном формате"
            )
    ...
    with allure.step("Анализируем визуальные свойства прогресс-бара"):
        ...
        allure.attach(
            str(visual_properties),
            "progress_bar_visual_properties",
            allure.attachment_type.JSON,
        )
        ...
        visual_checks = {
            ...
            "has_background_color": visual_properties.get("background_color")
            is not None,
        ...
        assert (
            visual_checks["has_width"] and visual_checks["has_height"]
        ), "Прогресс-бар должен иметь размеры"
    ...
    with allure.step("Проверяем соответствие визуального заполнения значению"):
        ...
        fill_analysis = {
            ...
            "values_approximately_match": (
                abs(visual_fill_percentage - progress_percentage) <= 10
                if isinstance(progress_percentage, (int, float))
                else False
            ),
            "visual_representation_accurate": (
                visual_fill_percentage > 0 if progress_percentage > 0 else True
            ),
        }

        progress_bar_page.log_step(f"Анализ визуального заполнения: {fill_analysis}")
        allure.attach(
            str(fill_analysis), "visual_fill_analysis", allure.attachment_type.JSON
        )

        if fill_analysis["values_approximately_match"]:
            progress_bar_page.log_step(
                "✅ Визуальное заполнение соответствует заявленному значению"
            )
        elif fill_analysis["visual_representation_accurate"]:
            progress_bar_page.log_step("✅ Визуальное представление корректное")
        else:
            progress_bar_page.log_step(
                "ℹ️ Визуальное заполнение может не точно соответствовать значению"
            )
...
def test_dynamic_progress_bar_animation(progress_bar_page: ProgressBarPage):
    ...
    with allure.step("Проверяем наличие динамического прогресс-бара"):
        ...
        progress_bar_page.log_step(
            f"Динамический прогресс-бар присутствует: {dynamic_progress_present}"
        )

        assert (
            dynamic_progress_present
        ), "Динамический прогресс-бар должен присутствовать на странице"
    ...
    with allure.step("Получаем начальное состояние динамического прогресс-бара"):
        ...
        allure.attach(
            str(initial_state),
            "initial_dynamic_progress_state",
            allure.attachment_type.JSON,
        )
        ...
        if start_button_available:
            ...
            if start_result:
                ...
                for i in range(6):  # 6 замеров с интервалом в 1 секунду
                    ...
                    current_percentage = (
                        progress_bar_page.get_dynamic_progress_percentage()
                    )
                    ...
                    progress_bar_page.log_step(
                        f"Snapshot {i + 1}: {current_percentage}% - {current_text}"
                    )
            ...
            for i in range(4):
                ...
                progress_snapshots.append(snapshot)
                progress_bar_page.log_step(
                    f"Observation {i + 1}: {current_percentage}%"
                )

    with allure.step("Анализируем динамику изменения прогресса"):
        allure.attach(
            str(progress_snapshots), "progress_snapshots", allure.attachment_type.JSON
        )
        ...
        if len(progress_snapshots) >= 2:
            ...
            increasing_trend = all(
                progress_snapshots[i]["percentage"]
                >= progress_snapshots[i - 1]["percentage"]
                for i in range(1, len(progress_snapshots))
                if isinstance(progress_snapshots[i]["percentage"], (int, float))
                and isinstance(progress_snapshots[i - 1]["percentage"], (int, float))
            )
            ...
            dynamic_analysis = {
                ...
                "total_progress_change": (
                    last_snapshot["percentage"] - first_snapshot["percentage"]
                    if isinstance(last_snapshot["percentage"], (int, float))
                    and isinstance(first_snapshot["percentage"], (int, float))
                    else 0
                ),
                "progress_increased": (
                    last_snapshot["percentage"] > first_snapshot["percentage"]
                ...
                "progress_animated": len(
                    set(
                        snap["percentage"]
                        for snap in progress_snapshots
                        if isinstance(snap["percentage"], (int, float))
                    )
                )
                > 1,
        ...
        progress_bar_page.log_step(
            f"Анализ динамического прогресса: {dynamic_analysis}"
        )
        allure.attach(
            str(dynamic_analysis),
            "dynamic_progress_analysis",
            allure.attachment_type.JSON,
        )

        if dynamic_analysis["progress_animated"]:
            progress_bar_page.log_step(
                "✅ Динамический прогресс-бар показывает анимацию"
            )
        else:
            progress_bar_page.log_step(
                "ℹ️ Изменения прогресса не обнаружены или минимальны"
            )
...
def test_progress_bar_controls(progress_bar_page: ProgressBarPage):
    ...
    with allure.step("Анализируем доступные элементы управления"):
        ...
        progress_bar_page.log_step(
            f"Доступные элементы управления: {available_controls}"
        )

        allure.attach(
            str(available_controls),
            "available_progress_controls",
            allure.attachment_type.JSON,
        )
    ...
    with allure.step("Тестируем кнопку Start/Play"):
        if available_controls.get("start_button", False):
            ...
            start_test = {
                ...
                "progress_started": (
                    state_after_start != start_initial_state
                    if isinstance(state_after_start, (int, float))
                    and isinstance(start_initial_state, (int, float))
                    else False
                ),
            }
    ...
    with allure.step("Тестируем кнопку Stop/Pause"):
        if available_controls.get("stop_button", False):
            ...
            stop_test = {
                ...
                "progress_stopped": (
                    state_after_stop == state_after_delay
                    if isinstance(state_after_stop, (int, float))
                    and isinstance(state_after_delay, (int, float))
                    else False
                ),
            }
    ...
    with allure.step("Тестируем кнопку Reset"):
        if available_controls.get("reset_button", False):
            ...
            reset_test = {
                ...
                "progress_reset_to_zero": (
                    state_after_reset == 0
                    if isinstance(state_after_reset, (int, float))
                    else False
                ),
                "progress_reset_to_minimum": (
                    state_after_reset <= 5
                    if isinstance(state_after_reset, (int, float))
                    else False
                ),
    ...
    with allure.step("Анализируем функциональность элементов управления"):
        allure.attach(
            str(control_tests), "progress_controls_tests", allure.attachment_type.JSON
        )

        successful_controls = sum(
            1 for test in control_tests if test["click_successful"]
        )
        functional_controls = sum(
            1
            for test in control_tests
            if test.get("progress_started")
            or test.get("progress_stopped")
            or test.get("progress_reset_to_zero")
            or test.get("progress_reset_to_minimum")
        )
        ...
        controls_summary = {
            ...
            "control_success_rate": (
                successful_controls / len(control_tests) if control_tests else 0
            ),
            "functional_rate": (
                functional_controls / len(control_tests) if control_tests else 0
            ),
            "control_details": control_tests,
        }

        progress_bar_page.log_step(
            f"Итоги тестирования элементов управления: {controls_summary}"
        )
        allure.attach(
            str(controls_summary),
            "progress_controls_summary",
            allure.attachment_type.JSON,
        )
        ...
        if controls_summary["controls_work"]:
            ...
            progress_bar_page.log_step(
                "⚠️ Элементы управления недоступны или не функционируют"
            )

        if controls_summary["controls_affect_progress"]:
            progress_bar_page.log_step(
                "✅ Элементы управления влияют на состояние прогресса"
            )
        else:
            progress_bar_page.log_step(
                "ℹ️ Влияние элементов управления на прогресс не обнаружено"
            )
...
def test_multiple_progress_bars(progress_bar_page: ProgressBarPage):
    ...
    with allure.step("Подсчитываем количество прогресс-баров на странице"):
        ...
        progress_bar_page.log_step(
            f"Найдено прогресс-баров на странице: {progress_bars_count}"
        )

        if progress_bars_count <= 1:
            progress_bar_page.log_step(
                "ℹ️ На странице только один прогресс-бар или они не найдены"
            )
            return
    ...
    with allure.step("Проверяем независимость прогресс-баров"):
        ...
        if progress_bars_count >= 1:
            interaction_result = progress_bar_page.interact_with_progress_bar_by_index(
                0
            )
            progress_bar_page.page.wait_for_timeout(2000)
            ...
            independence_analysis = {
                ...
                "first_bar_changed": (
                    states_after_interaction[0] != initial_states[0]
                    if len(states_after_interaction) > 0 and len(initial_states) > 0
                    else False
                ),
                "other_bars_unchanged": all(
                    states_after_interaction[i] == initial_states[i]
                    for i in range(1, len(initial_states))
                    if isinstance(states_after_interaction[i], (int, float))
                    and isinstance(initial_states[i], (int, float))
                ),
            ...
            if (
                independence_analysis["first_bar_changed"]
                and independence_analysis["other_bars_unchanged"]
            ):
            ...
            progress_bar_page.log_step(
                f"Анализ независимости прогресс-баров: {independence_analysis}"
            )
            allure.attach(
                str(independence_analysis),
                "progress_bars_independence",
                allure.attachment_type.JSON,
            )
    ...
    with allure.step("Анализируем типы и стили прогресс-баров"):
        types_analysis = {
            "static_bars": sum(
                1 for info in all_progress_bars_info if info.get("type") == "static"
            ),
            "dynamic_bars": sum(
                1 for info in all_progress_bars_info if info.get("type") == "dynamic"
            ),
            "different_styles": len(
                set(info.get("style", "default") for info in all_progress_bars_info)
            ),
            "different_colors": len(
                set(info.get("color", "default") for info in all_progress_bars_info)
            ),
            "all_visible": all(
                info.get("visible", False) for info in all_progress_bars_info
            ),
            "all_functional": all(
                info.get("functional", False) for info in all_progress_bars_info
            ),
        }
    ...
    with allure.step("Создаем итоговый отчет о множественных прогресс-барах"):
        multiple_bars_summary = {
            ...
            "independence_confirmed": independence_analysis.get(
                "bars_are_independent", None
            ),
            "variety_in_styles": types_analysis["different_styles"] > 1,
            "all_bars_visible": types_analysis["all_visible"],
            "multiple_bars_functional": progress_bars_count > 1
            and types_analysis["all_visible"],
        }

        progress_bar_page.log_step(
            f"Итоги множественных прогресс-баров: {multiple_bars_summary}"
        )
        allure.attach(
            str(multiple_bars_summary),
            "multiple_progress_bars_summary",
            allure.attachment_type.JSON,
        )

        assert multiple_bars_summary[
            "multiple_bars_functional"
        ], f"Множественные прогресс-бары должны быть функциональными: найдено {progress_bars_count}, видимы {types_analysis['all_visible']}"

        if multiple_bars_summary["independence_confirmed"]:
            progress_bar_page.log_step(
                "✅ Прогресс-бары работают независимо друг от друга"
            )
        elif multiple_bars_summary["independence_confirmed"] is False:
            progress_bar_page.log_step(
                "ℹ️ Прогресс-бары могут быть связаны друг с другом"
            )
        else:
...
def test_progress_bar_performance(progress_bar_page: ProgressBarPage):
    ...
    with allure.step("Подготавливаем прогресс-бар для тестирования производительности"):
        ...
        progress_bar_page.log_step(
            f"Начальное состояние для теста производительности: {initial_performance_state}"
        )
    ...
    with allure.step("Запускаем прогресс и измеряем производительность"):
        if progress_bar_page.is_start_progress_button_available():
            ...
            for i in range(10):  # 5 секунд измерений
                ...
                progress_bar_page.log_step(
                    f"Метрика {i + 1}: прогресс {current_progress}%, отклик {ui_response_time:.2f}мс"
                )

    with allure.step("Анализируем метрики производительности"):
        allure.attach(
            str(performance_metrics), "performance_metrics", allure.attachment_type.JSON
        )

        if performance_metrics:
            # Анализ времени отклика UI
            avg_response_time = sum(
                m["ui_response_time_ms"] for m in performance_metrics
            ...
            progress_values = [
                ...
                if isinstance(m["progress_value"], (int, float))
            ]
            progress_is_smooth = (
                len(set(progress_values)) > len(progress_values) * 0.5
                if progress_values
                else False
            )

            # Анализ стабильности
            response_time_variance = max(
                m["ui_response_time_ms"] for m in performance_metrics
            ) - min(m["ui_response_time_ms"] for m in performance_metrics)
            ...
            performance_analysis = {
                ...
                "performance_acceptable": avg_response_time < 100
                and response_time_variance < 200,
            }
        ...
        if performance_analysis["performance_acceptable"]:
            ...
            progress_bar_page.log_step(
                "⚠️ Производительность прогресс-бара может требовать оптимизации"
            )
        ...
        assert (
            len(performance_metrics) > 0
        ), "Должны быть собраны метрики производительности"
