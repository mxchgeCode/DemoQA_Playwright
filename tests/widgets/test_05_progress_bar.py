...
def test_progress_bar_stops_and_resets(progress_page):
    ...
    assert (
        progress_page.get_progress_value() == "0%"
    ...
    try:
        wait_for_progress(progress_page, "50%", timeout=90)
    ...
    progress_page.page.wait_for_timeout(5000)
    ...
    progress_page.page.wait_for_timeout(1000)
    try:
        wait_for_progress(progress_page, "100%", timeout=45)
    except TimeoutError:
        current_value = progress_page.get_progress_value()
        current_percent = int("".join(filter(str.isdigit, current_value)))
