...
def test_2_select_value_functionality(select_menu_page):
    ...
    expected_options = ["Group 1, option 1", "Group 1, option 2", "Group 2, option 1", "Group 2, option 2", "Another root option"]
    found_options = []
    for option_text in expected_options:
        option_locator = select_menu_page.page.locator("div[class*='option']").filter(has_text=option_text)
...
def test_6_standard_multi_select_functionality(select_menu_page):
    ...
    assert select_menu_container.count() > 0, "Контейнер #selectMenuContainer должен существовать"
    assert select_menu_container.is_visible(), "Контейнер #selectMenuContainer должен быть видим"
    ...
    potential_options = ["Group 1, option 1", "Group 2, option 1", "Aqua", "Blue", "Green", "Red"]
    found_options = []
    for option_text in potential_options:
        option_locator = select_menu_page.page.locator(f"div[class*='option']:has-text('{option_text}')")
        if option_locator.count() > 0 and option_locator.first.is_visible():
            found_options.append(option_text)
    print(f"✓ Найдено {len(found_options)} потенциальных опций в Standard Multi Select dropdown: {found_options}")

    # Утверждение: хотя бы две опции должны быть найдены
    assert len(
        found_options) >= 2, f"В dropdown Standard Multi Select должны быть как минимум 2 опции из {potential_options}"
    ...
    if len(found_options) >= 2:
        ...
        assert len(selected_values) >= 2, "Должно быть выбрано хотя бы два значения"
        assert option_to_select_1 in selected_values, f"Выбранное значение '{option_to_select_1}' должно быть в списке"
        assert option_to_select_2 in selected_values, f"Выбранное значение '{option_to_select_2}' должно быть в списке"
