...
class ResizableLocators:
    ...
    RESIZABLE_BOX_RESTRICTED = "#resizableBoxWithRestriction"  # Добавляем отсутствующий локатор
    RESIZABLE_BOX_NO_RESTRICTION = "#resizable"  # Добавляем отсутствующий локатор
    BOX_HANDLE = "#resizableBoxWithRestriction > span.react-resizable-handle-se"  # Ручка блока с ограничениями
    RESIZE_HANDLE_RESTRICTED = "#resizableBoxWithRestriction > span.react-resizable-handle-se"  # Добавляем отсутствующий локатор
    RESIZE_HANDLE_NO_RESTRICTION = "#resizable > span.react-resizable-handle-se"  # Добавляем отсутствующий локатор

    BUTTON_RESIZABLE = "#resizable"  # Кнопка без ограничений размера
    BUTTON_HANDLE = (
        "#resizable > span.react-resizable-handle-se"  # Ручка кнопки без ограничений
    )
